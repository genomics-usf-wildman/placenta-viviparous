...
<<load.data,echo=FALSE,results="hide",cache.extra=tools::md5sum(paste0("../data/",c("combined_fpkm","gene_trees","housekeeping_genes_superset","placenta_core_transcriptome","placenta_go_results_bp","placenta_go_results_mf","hugo_nomenclature")))>>=
...
\textit{Mus musculus} \cite{Wang.ea2011:surveyfornovelimprinted},
\textit{Homo sapiens}
\cite{Hughes.ea2015:Evaluatingintra-andinter-individual} \textit{Equus
  caballus} \DLA{citation}, \textit{Ovis aries} \DLA{citation}, and
\textit{Sus scrofa} \DLA{citation} were obtained from the \ac{SRA}
(Supplemental \cref{tab:srr_numbers}).
...
gene lists were compared using the \textit{Homo sapiens} Gene Ontology
using the 1:1 \textit{Homo sapiens} ortholog.

\subsection{Comparative Analysis of Gene Expression}

All 1:1 orthologues present among \textit{D. novemcinctus}, \textit{P.
  troglodytes}, \textit{B. taurus}, \textit{C. familiaris}, \textit{L.
  africana}, \textit{M. musculus}, \textit{H. sapiens}, and \textit{M.
  domestica} were determined using Ensembl v.80 gene trees via
