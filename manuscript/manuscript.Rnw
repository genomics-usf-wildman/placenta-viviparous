...
\acrodef{FPKM}{Fragments Per Kilobase of exon per Million reads \cite{Trapnell.ea2010:Transcriptassemblyandquantification}}
...
<<load.data,echo=FALSE,results="hide",cache.extra=tools::md5sum(paste0("../data/",c("combined_fpkm")))>>=
for (a in paste0("../data/",c("combined_fpkm"))) {
    load(a)
}
@ 
...
values in \ac{FPKM} were quantified using Cufflinks v2.2.1
...
\begin{table}
  \begin{tabular}{c c c c}
    \toprule
    Species                       & Common Name        & Reference & Annotation \\
    \midrule
    \textit{Dasypus novemcinctus} & 9-Banded Armadillo & DASNOV3    & Ensembl 80 \\
    \textit{Ateles fusciceps}     & Spider monkey      & ---        & \textit{Homo sapiens} Ensembl 80 \\
    \textit{Pan paniscus}         & Bonobo             & CHIMP2.1.4 & Ensembl 80 \\
    \textit{Bos taurus}           & Cow                & UMD3.1     & Ensembl 80 \\
    \textit{Canis familiaris}     & Dog                & CanFam3.1  & Ensembl 80 \\
    \textit{Loxodonta africana}   & Elephant           & loxAfr3    & Ensembl 80 \\
    \textit{Homo sapiens}         & Human              & GRCh38     & Ensembl 80 \\
    \textit{Mus musculus}         & Mouse              & GRCm38     & Ensembl 80 \\
    \textit{Nannospalax galili}   & Blind mole rat     & ----       & \textit{Mus musculus} Ensembl 80 \\
    \textit{Monodelphis domestica} & Opossum           & BROAD05    & Ensembl 80 \\
    \textit{Spalax carmeli}       & Blind mole rat     & ----       & \textit{Mus musculus} Ensembl 80 \\
    \bottomrule
  \end{tabular}
...
top.genes.table <- function(species.name,n=50) {
    temp <- combined.fpkm[species==species.name &
                      !grepl("^MT-?",gene_short_name)][order(-mean_fpkm)][!duplicated(gene_short_name)][1:n,list(gene_short_name,human_name,mean_fpkm)]
...
@ 
\caption{Expression in \ac{FPKM} of top 50 non-mitochondrial genes in
  \textit{Homo sapiens}}
\label{tab:top50_human_placenta}
\end{wraptable}

The top 50 expressed genes as measured by \ac{FPKM} in the term placenta of
the human are shown in \fref{tab:top50_human_placenta} with a list of all genes in
Supplemental Table X. The gene with by far the highest \ac{FPKM} value
...
GH2 (\ac{FPKM}=729.639), is also expressed to a high degree in the human
...
PSG4 showing the greatest degree of expression (\ac{FPKM}= 5201.31; Table
...
(\ac{FPKM} = 4817.15 and 829.846 respectively), as well as CGA (\ac{FPKM}=
...
top.human.genes <- top.genes.table(species.name="pan paniscus")
print(xtable(top.human.genes),
      booktabs=TRUE,
      floating=FALSE
      )
...
50 genes (Table 3). The highest expression (\ac{FPKM}=10149.1) in bonobo is
...
human placenta (\ac{FPKM}=250.675; Supplemental Table X), although at
...
expressed at high levels (\ac{FPKM}=1232.56; Table 3). Two other Pan genes
