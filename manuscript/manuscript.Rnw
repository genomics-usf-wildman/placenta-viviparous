...
opts_chunk$set(dev="cairo_pdf",out.width="\\columnwidth",out.height="0.7\\textheight",out.extra="keepaspectratio,center")
...
pathway <- list()
for (database in c("kegg","biocarta","netpath")) {
...
\author[affnih,affasan]{Chong J. Kim}
\ead{ckim@amc.seoul.kr}
...
\fntext[n1]{Equal contributor}
\address[affigb]{Carl R. Woese Institute for Genomic Biology, University of Illinois at Urbana-Champaign, Urbana, IL, USA}
...
\address[affasan]{Department of Pathology, University of Ulsan College of Medicine, Asan Medical Center, Seoul, Korea}
...
\begin{abstract}
  ...
  core genes which is expressed (FPKM $\ge 10$) in all eutherian placentas, %
  %
  including genes with immune-modulating properties (\acs{ANXA2},
  \acs{ANXA1}, \acs{S100A11}, \acs{S100A10}, and \acs{LGALS1}), cell-cell
  interactions (\acs{LAMC1}, \acs{LUM}, and \acs{LGALS1}),
  invasion (\acs{GRB2} and \acs{RALB}) and syncytialization
  (\acs{ANXA5} and \acs{ANXA1}). %
  ...
  morphology, including \acs{EREG} and \acs{WNT5A} which are both
...
mammals, the placenta varies widely in terms of shape, degree
...
genes which are responsible for the \textbf{core functions of the
...
For example, each superorder (Euarchontoglires, Laurasiatheria,
Afrotheria, and Xenartha, \cref{fig:species_tree}) has evolved
expansive placenta-specific gene families with expression patterns
...
and \cow, Pregnancy-Specific Glycoproteins in \human and \mouse,
Pregnancy-Associated Glycoproteins in \cow, genes related to the
thymus-specific serine protease \ac{PRSS16} in \elephant, and the
expansion of the growth hormone and chorionic gonadotropin beta gene
families in anthropoid primates
...
including \human [human], \bonobo [bonobo], (the sister
species to \textit{Pan troglodytes}), \smonkey [spider monkey],
\mouse [mouse], \galili [blind mole rat from Upper Galilee], \carmeli [blind mole rat from northern Israel], \dog [domestic dog], \cow [domestic cow], \elephant [African savanna elephant],
\armadillo [nine-banded armadillo], and \opossum [gray short-tailed opossum].
...
as marsupials (\cref{fig:species_tree}). These placentas also represent a wide array of
placental morphologies as well as traits such as number and size of
offspring and length of gestation (Supplemental \cref{tab:gestational_periods}). We
use these data, along with existing RNA-Seq data from \mouse 
...
\pig [domestic pig] \cite{Ahanda.ea2012:Predictionofaltered3-} to: 1)
...
\textbf{core functionality of the placenta}; 2) identify genes whose
expression level changes significantly in each of the placenta morphology
types.
...
Fetal tissue was collected from the placenta of nine mammalian species
(\human [human], \bonobo [bonobo], \smonkey [spider monkey], \dog
[domestic dog], \cow [domestic cow], \elephant [African savanna
elephant], \opossum [gray short-tailed opossum], \galili [blind mole
rat from Upper Galilee], \carmeli [blind mole rat from northern
Israel] \cite{Nevo2001:adaptiveradiationofblindsubterraneanmole}; n=1
for all collected species). Information on specimen ID, number of
specimens, and collector is shown for each sample in Supplemental
...
was collected upon the death of the mother and fetus from a car
accident. Placental tissue from \opossum was sampled at fetal stage
32, roughly one day prior to birth. Only the fetal portion of
the placenta was sampled at the maternal-fetal interface (i.e. villous
portion for \human, \bonobo, \dog, and \elephant; cotyledon for \cow;
diffuse placenta for \opossum). The placenta membranes were separated
from the fetal portion by dissection, and the
resulting fetal portion was washed twice in PBS. All tissues
were collected and stored in RNAlater (Life Technologies, Carlsbad,
CA) until RNA isolation could be performed. Tissues were homogenized
in Trizol, and total RNA was isolated following the manufacturer's
instructions. RNA was further purified using RNeasy Midi Kit (Qiagen,
Valencia, CA), in conjunction with the RNase-Free DNase Set (Qiagen,
Valencia, CA), following the manufacturer’s protocol to remove any
contaminating DNA from the sample. Total RNA was then evaluated on a
Nanodrop 1000 (Thermo Scientific, Wilmington, DE) and an Agilent
Bioanalyzer 2100 (Santa Clara, CA) to confirm concentration and
quality of the isolation. RNA quality was assessed using A260/A280 and
A260/A230 absorbance values.
...
\subsubsection{Additional Placenta Transcriptomes}

For more complete evolutionary comparisons additional transcriptomes of
...
\cref{fig:transcriptome_statistics,fig:transcriptome_reads_vs_read_length,tab:ref_genome_aligned_species})
...
species.tree <- 
    read.tree(text="(Monodelphis_domestica:7,((Dasypus_novemcinctus:5,Loxodonta_africana:5)Atlantogenata:1,((Canis_familiaris:4,(Equus_caballus:3,(Sus_scrofa:2,(Ovis_aries:1,Bos_taurus:1):1):1):1)Laurasiatheria:1,(((Spalax_carmeli:1,Spalax_galili:1):1,Mus_Musculus:2)Rodentia:2,((Pan_paniscus:1,Homo_sapiens:1):1,Ateles_fuciceps:2)Primates:2)Euarchontoglires:1)Boreoeutheria:1)Eutheria:1);")
pp <- ggtree(species.tree)+
    geom_text(aes(label=gsub("_"," ",label)),nudge_x=0.05,vjust=0.65,hjust=0,size=4,fontface="italic")+
    geom_text(data=data.frame(x=1,y=1),label="Metatheria",nudge_y=0.05,hjust=0,vjust=0,size=4,fontface="italic")+
    geom_text(data=data.frame(x=3.3,y=c(2,3)),label=c("Xenartha","Afrotheria"),nudge_y=0.05,hjust=0,vjust=c(0,0),size=4,fontface="italic")+
...
print(pp %>%
      ...
      ## Armadillo
      %>% phylopic("5d59b5ce-c1dd-40f6-b295-8d2629b9775e", color="#44EEBB",alpha=0.3,node=2,width=0.13)
      ## elephant
      %>% phylopic("80db1004-bc9f-4318-84e7-cdd9639a1f3e",color="#BB1111",alpha=0.3,node=3,width=0.13)
      ## dog
      ...
      ## mouse
      %>% phylopic("6b2b98f6-f879-445f-9ac2-2c2563157025",color="#BB11CC",alpha=0.3,node=11,width=0.15)
      ## bonobo
      %>% phylopic("f06b4dd0-094e-4f37-9105-25fdb4eb1b02",color="#1111FF",alpha=0.3,node=12,width=0.13)
      ## spider monkey
...
\caption{Species tree for organisms studied showing the relationship
  between species. Branch lengths are not to scale. Silhouettes of
  species were obtained from \url{http://phylopic.org}. }
...
Orthologous pairs between \human and all other species were identified
using the \ac{OMA} database
\cite{Altenhoff.ea2015:OMAorthologydatabasein}. In cases where a gene
did not have a human ortholog pair identified by \ac{OMA}, Ensembl v.80
one2one orthologs obtained from Biomart using the biomaRt R package
\cite{Durinck.ea2009:Mappingidentifiersforintegration} %
were used to select the appropriate orthologous pair. A total of
\Sexpr{combined.fpkm[,list(n=length(gene_id),
  max.fpkm=max(mean_fpkm)),by=human_name][n==14 & max.fpkm >
  10,length(human_name)]} ortholog pair sets in all 14 species were
expressed (\ac{FPKM} $\ge 10$) in at least one species. We also
obtained orthology from the perspective of each species to all other
species using the biomaRt package and the Ensembl v.80 database to determine
the percentage of highly expressed genes that had an orthology of
one-to-one, one-to-many, many-to-many, or novel/unknown. For future
analyses by other investigators, we have also provided the human gene
which most closely aligns to each expressed annotated
protein-generating sequence of all species analyzed using Diamond
\cite{Buchfink.ea2015:Fastandsensitiveprotein}.

Significant shifts in expression levels between groups of species were
calculated using Student's t test on ortholog pairs using the mean of
...
are reported in $\log_2$ Fold-Change (FC).
...
To identify genes which have evidence of association with adverse
pregnancy outcomes and other placenta-relevant terms of interest, we
used the “pubmed\_to\_grid” component of
Function2Gene~\cite{Armstrong2008:function2gene} which conducts a term
by gene search of PubMed results.

\subsection{Sequences, FPKMs, and Code}

All of the code used to produce every analysis presented in this paper
and the manuscript itself is available at
\url{https://github.com/uiuc-cgm/placenta-viviparous.git}. All of the
sequences and assembled transcriptomes used are deposited in SRA, and
new sequences for this work can be obtained under the series
\href{http://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE79121}{GSE79121}
(Supplemental \cref{tab:srr_numbers}). The mean \ac{FPKM} for gene in
each species with ortholog annotations are located in the supplemental
data file all\_species\_mean\_fpkm.txt.xz and the per-sample \ac{FPKM}
values are in all\_species\_per\_sample\_fpkm.txt.xz.

\section{Results}

\subsection{Transcriptome Statistics}

Supplemental \cref{fig:transcriptome_statistics} shows the RNA-Seq read and
...
\cref{fig:transcriptome_reads_vs_read_length}). %
%
Between 20\% and 37\% of all highly expressed genes in each species
were ortholog pairs with all other analyzed species, with the
remainder being mostly one-to-many or many-to-many in comparison to at
least one other species (Supplemental
\cref{tab:orthologs_in_each_species}).
%
The expression of
...
<<load.placenta.lineage.specific,echo=FALSE,results="hide",cache.extra=tools::md5sum(c(paste0("../data/",c("placenta_lineage_specific","human_mouse_lineage_specific")),"../literature_mining/human_lineage_vs_terms.txt"))>>=
...
literature.mining <-
    data.table(read.table("../literature_mining/human_lineage_vs_terms.txt",
                          sep="\t",skip=0,stringsAsFactors=FALSE,
                          check.names=FALSE,header=TRUE),
               keep.rownames=TRUE)
setnames(literature.mining,"rn","human_name")
setkey(literature.mining,"human_name")
literature.mining[,`Adverse Pregnancy Outcomes`:=`preeclampsia`+
                       IUGR+`placenta previa`+`gestational diabetes`+
                       `premature birth`,
                  by=human_name]
literature.mining.terms <-
    paste0("%28%22preeclampsia%22[All%20Fields]%20OR%20",
           "%22pre-eclampsia%22[All%20Fields]%20OR%20",
           "%22eclampsia%22[All%20Fields]%20OR%20",
           "%22IUGR%22[All%20Fields]%20OR%20",
           "%22iugr%22[All%20Fields]%20OR%20",
           "%22intrauterine%20growth%20restriction%22[All%20Fields]%20OR%20",
           "%22placenta%20previa%22[All%20Fields]%20OR%20",
           "%22gestational%20diabetes%22[All%20Fields]%20OR%20",
           "%22premature%20birth%22[All%20Fields]%20OR%20",
           "%22premature%20labor%22[All%20Fields]%29")
placenta.lineage.specific.table <- function(analysis,output=TRUE,lineage.table=placenta.lineage.specific.analysis,members=5,orderbyfc=FALSE,fdr.cutoff=1) {
    ...
    temp.table <-
        literature.mining[,list(human_name,`Adverse Pregnancy Outcomes`)
                          ][temp.table]
    temp.table[,
               `Adv. Preg. Publications`:="0"
               ]
    temp.table[`Adverse Pregnancy Outcomes` >= 2,
               `Adv. Preg. Publications`:="≥ 2"
               ]
    temp.table[`Adverse Pregnancy Outcomes` >= 5,
               `Adv. Preg. Publications`:="≥ 5"
               ]
    temp.table[`Adverse Pregnancy Outcomes` >= 20,
               `Adv. Preg. Publications`:="≥ 20"
               ]
    temp.table[`Adverse Pregnancy Outcomes` == 1,
               `Adv. Preg. Publications`:="1"
               ]
    setkey(temp.table,"human_name")
    temp.table <- 
        hugo.nomenclature[temp.table][,list(`Approved Symbol`,
                                            `Approved Name`,p.value,fdr,fc,
                                            `Adv. Preg. Publications`
                                            )]
    setnames(temp.table,
    ...
    if (output) {
        ...
        display.table <-
            display.table[,`Adv. Preg. Pub.`:=
                               paste0("\\href{https://www.ncbi.nlm.nih.gov/pubmed/?term=%22",
                                      `Human Symbol`,
                                      "%22[All%20Fields]%20AND%20",
                                      literature.mining.terms,"}{",
                                      `Adv. Preg. Pub.`,"}"
                                      )]
        display.table <-
            display.table[,`Human Symbol`:=
                               paste0("\\href{http://www.ensembl.org/Homo_sapiens/Gene/Summary?g=",
                                      `Human Symbol`,"}{",
                                      "\\textit{",xtable.sanitize.text(`Human Symbol`),
                                      "}}")]
        display.table[,`Human Name`:=NULL]
        # setnames(display.table,"Human Symbol","\\multicolumn{1}{p{0.5in}}{Human Symbol}")
        setnames(display.table,"Fold Change","{$\\text{log}_2$ FC}")
        setnames(display.table,"FDR","{FDR}")
        setnames(display.table,"p (t-test)","{p (t-test)}")
        print(xtable(display.table,
                     display=c("s","s","e","e","f","s"),
                     align=c("c","p{0.5in}","S[table-format=1.2e2]","S[table-format=1.2e2]","S[table-format=2.2]","p{0.5in}")
                     ),
              booktabs=TRUE,
              floating=FALSE,
...
\subsection{\human Lineage Specific Genes and Their Association with Pre-eclampsia}
...
@ 
\caption{\lineagetableexplanationtypeiiishort{\human}{all other
    analyzed species} The human symbol links to a brief explanation of
  the gene function in Ensembl. Adv. Preg. Pub. is the approximate
  number of publications indexed by PubMed mentioning this gene and an
  adverse pregnancy outcome (pre-eclampsia, premature labor,
  gestational diabetes, intrauterine growth restriction, or premature
  birth) as of February 2017, and links to the most current PubMed
  search results.}
\label{tab:human_lineage_specific}
\end{table}

Using a type-III ANOVA to account for repeated testing of
\human (see \cref{sec:comp-analys-gene} for details), we identified multiple genes which are
...
\subsection{Other Lineage and Clade Specific Genes}

We also compared differences in the \mouse lineage in comparison to
all other lineages using a type-III ANOVA as in the \human analysis,
and found few genes which are highly differentially expressed and are
also associated with adverse pregnancy outcomes, including \ac{APOB}
and \ac{AFP} (Supplemental \cref{tab:mouse_lineage_specific_all}).
Comparisons between rodentia (\mouse, \galili, and \carmeli) and all
other species using the mean of each species expression did not
indicate any significant differences (Supplemental
\cref{tab:rodentia_lineage_specific}) nor did the equivalent
comparisons between laurasiatheria and all other species (Supplemental
\cref{tab:laurasiatheria_lineage_specific}) and laurasiatheria and
euarchontoglires (Supplemental
\cref{tab:laurasiatheria_euarchontoglires_lineage_specific}).
Additional placenta samples of each species and the collection of the
placentas of additional laurasiatherians may resolve the lack of
significance due to insufficient power.

\subsection{Core Placenta Transcriptome}

<<core.placenta.transcriptome.distribution.calc,echo=FALSE,message=FALSE,results="hide">>=
...
\begin{figure}[tbh]
<<core.placenta.transcriptome,fig.width=19/2.5,fig.height=6,out.width="190mm",echo=FALSE>>=
cptl <-
...
cptl <- cptl[,median_FPKM:=median(FPKM),by=human_name]
cptl <- cptl[,facet:="high"]
cptl <- cptl[median_FPKM < median(median_FPKM),facet:="low"]
print(ggplot(cptl[FPKM > 0.1,],aes(x=human_name,y=FPKM))+
      geom_boxplot(outlier.shape=NA)+
      scale_y_log10(breaks=c(10^1,10^2,10^3,10^4),
                    labels=trans_format("log10", math_format(10^.x)))+
      geom_point(aes(color=Species,shape=Species))+
      scale_shape_manual(NULL,##"Species",
...
\caption{Core Placenta Transcriptome.
  \Sexpr{core.placenta.transcriptome.long[housekeeping==FALSE,length(unique(human_name))]}
  ortholog pairs of \human genes expressed (\ac{FPKM}$ \ge 10$) in the
  placenta of all 13 eutherians studied with housekeeping genes
  \cite{Eisenberg.ea2013:Humanhousekeepinggenesrevisited} removed
  ordered (from top to bottom, left to right) by median expression. If we also require
  expression (\ac{FPKM}$ \ge 10$) in the marsupial, \opossum, there are 
  \Sexpr{cpt.distribution.shape[`Expression (FPKM)` > 10 &
    `Min. Exp. in` == '14 of 14 Species',max(num_genes)]} genes
  ...
  transcriptome if different \ac{FPKM} and species restriction criteria are
  utilized.}
...
<<core.placenta.transcriptome.distribution,fig.width=9/2.5,fig.height=9/2.5,out.width="90mm",out.height="0.8\\textheight",echo=FALSE,message=FALSE>>=
  print(ggplot(cpt.distribution.shape[`Expression (FPKM)` > 0.1,],
      ...
      + geom_segment(mapping=aes(y=V1-ifelse(V1<100,20,0),
                         yend=V1,
                         xend=10,
                         x=sapply(V1,
                                  function(x){
                                      cpt.distribution.shape[num_genes > x,
                                                             max(`Expression (FPKM)`)*2]
                                  }),
                         color=`Min. Exp. in`),
      ...
      + geom_text(mapping=aes(x=sapply(V1,
                                       function(x){
                                           cpt.distribution.shape[num_genes > x,
                                                                  max(`Expression (FPKM)`)*2.1]
                                       }),
                      y=V1-ifelse(V1<100,20,0),
                      label=paste0(V1," genes")),inherit.aes=FALSE,
                  data=cpt.distribution.shape[`Expression (FPKM)` > 10,
                      max(num_genes),`Min. Exp. in`],
                  show.legend=FALSE,
                  hjust=0,
...
@ 
\caption{Distribution shape of placenta core genes for all possible
  number of genes. The vertical axis
  shows the number of ortholog pairs included in the core
  placenta transcriptome (for all possible number of genes) if %
  \Sexpr{cpt.distribution.shape[,tolower(levels(`Min. Exp. in`))[1]]}
  %
  (or %
  \Sexpr{cpt.distribution.shape[,tolower(levels(`Min. Exp. in`))[2]]}, or
  \Sexpr{cpt.distribution.shape[,tolower(levels(`Min. Exp. in`))[3]]},
  % 
  ...
  %
  genes would be included if the expression of a set of ortholog pairs of all 13 Eutherians must be at
9  least 10 FPKM, %
  \Sexpr{cpt.distribution.shape[`Expression (FPKM)` > 10 
    & `Min. Exp. in` == '13 of 14 Species',max(num_genes)]}
  %
  ortholog pairs would be included if 13 of 14 species have an
...
<<load_core_placenta_tree_transcriptome,echo=FALSE,results="hide",cache.extra=file.info(paste0("../data/placenta_core_tree_transcriptome",c("","_permissive")))[,"mtime"]>>=
load("../data/placenta_core_tree_transcriptome")
load("../data/placenta_core_tree_transcriptome_permissive")
cpttl <-
    core.placenta.tree.transcriptome.long[!grepl("(min|median|max|all)_expression|all_but_",
                                                 Species) & housekeeping==FALSE,]
@ 
...
the placenta transcriptome if an ortholog pair of that gene was
expressed with $\text{FPKM} \ge 10$ in all eutherian species studied
(excluding the metatherian, \opossum) and was not a human housekeeping
gene according to
\citet{Eisenberg.ea2013:Humanhousekeepinggenesrevisited}. Genes which
meet the FPKM criterion and are housekeeping genes are depicted in
...
\Sexpr{cpt.distribution.shape[`Expression (FPKM)`>=1 & `Min. Exp. in`
  == 'All 13 Eutherians',max(num_genes)]} %
...
of the total fraction of genes (%
%
\Sexpr{core.placenta.transcriptome.long[housekeeping==TRUE,length(unique(human_name))]/
  core.placenta.transcriptome.long[,length(unique(human_name))]} %
%
%
vs %
\Sexpr{core.placenta.transcriptome.long[housekeeping==FALSE,length(unique(human_name))]/
  core.placenta.transcriptome.long[,length(unique(human_name))]}%
%
). An alternative approach to identify housekeeping genes is to use
the tissue specificity
index~\cite{Yanai.ea2005:Genome-widemidrangetranscriptionprofiles}.
Supplemental
\cref{tab:tissue_specificity_index_of_placenta_core_genes} lists the
tissue specificity index for all housekeeping and non-housekeeping
genes; housekeeping genes expressed in all placentas have a lesser
median tissue specificity index than non-housekeeping genes
(Supplemental \cref{fig:tissue_specificity_index_with_housekeeping}).
We also note that housekeeping genes have been defined in \human, and
may not be housekeeping genes in other species; generating tissue
expression atlases and examining the evolution of expression in
non-model organisms is a promising area of future research.
...
transcriptome by only requiring 10 of 14 species to express an
ortholog pair with FPKM $>10$ and results in the same terms also being
...
If we use gene trees to define the core placenta transcriptome instead
of ortholog pairs (where the mean expression for genes in the tree
which are present must be $\text{FPKM} \ge 10$), we find
\Sexpr{cpttl[,length(unique(tree_genes))]} unique gene trees which
include most
(\Sexpr{length(intersect(core.placenta.tree.human.genes.in.tree,
  core.placenta.transcriptome.long[,as.character(unique(human_name))]))/
  core.placenta.transcriptome.long[,length(unique(human_name))]*100}\%)
of the genes present in the core placenta transcriptome (Supplemental
\cref{fig:core_placenta_tree_transcriptome,fig:core_placenta_tree_transcriptome_housekeeping}).
Relaxing the mean gene tree expression requirement to
$\text{FPKM} \ge 5$ includes nearly all of the genes
(\Sexpr{length(intersect(core.placenta.tree.human.genes.in.permissive.tree,
  core.placenta.transcriptome.long[,as.character(unique(human_name))]))/
  core.placenta.transcriptome.long[,length(unique(human_name))]*100}\%).
...
transcriptome, including \ac{ANXA1}, \ac{ANXA2},
\ac{S100A10}, and \ac{S100A11}. ANXA1 is involved in the
...
known to interact with S100A4, S100A6, S100A10, and S100A11
...
\cite{Ma.ea2014:Down-regulationofTIMP3leads}.

Another annexin which is highly expressed in all term placentas is
\ac{ANXA5}, which was originally identified as an anti-coagulant
...
which are involved in multiple biological pathways (Supplemental
\cref{tab:core_placenta_pathway_analysis}). The most abundant set
of significantly enriched molecular pathways are related to the
migration of the trophoblast into the endometrium, including the EGFR1
pathway %
(\Sexpr{placpath("^EGFR1","Netpath")}), Focal Adhesion pathway
(\Sexpr{placpath("^EGFR1","Kegg")})
%
...
\subsection{Placenta Morphology: Shape}
...
\subsection{Placenta Morphology: Degree of Intimacy and Interdigitation}
...
\cref{tab:intimacy_genes,tab:interdigitation_genes}) using ANOVA, but
when we used an ordered factor analysis (proportional odds logistic
regression), none of these genes were retained (Supplemental
...
\ac{Afp} and \ac{Gjb2}. Furthermore, the biological processes which
ultimately result in pre-eclampsia occur well before term; all of our
placentas are term placentas and provide limited information about the
transcriptome during the onset of pre-eclampsia.
...
invasion (Supplemental
\cref{tab:core_placenta_go_terms,tab:core_placenta_pathway_analysis}).
...
are expressed in all placentas examined, and argue for the evolutionary
...
We suggest that further research into the molecular role of these annexin
complexes may illuminate how immune tolerance is maintained during
placentation, because annexins appear to be the major class of immune
modulatory transcripts which are expressed in all placentas analyzed.

\subsubsection{Formation of the syncytiotrophoblast}

The evolutionarily convergent capture of endogenous retroviruses to
form expressed fusogenic proteins such as Syncytin-1 and -2
(\ac{ERVW-1} and \ac{ERVFRD-1}, respectively) has been implicated in
syncytiotrophoblast formation in \human and many other
species~\cite{Rawn.ea2008:evolutionregulationandfunction}. While genes
which are similar to \ac{ERVFRD-1} and \ac{ERVW-1} are expressed in
most of the placentas analyzed here, there are no highly expressed
genes with sequence similarity in \bonobo and \dog, even though these
mammals form
...
and the syncytiotrophoblast must be continuously replenished, the
gene(s) responsible for cytotrophoblast fusion to form the
syncytiotrophoblast should be highly expressed in the core placenta.
\ac{ANXA5} is known to be expressed at multiple stages of
...
the presence of Ca$^{2+}$. We hypothesize that \ac{ANXA5} is essential
for the completion of fusion and the maintenance of membrane repair
during the flux of nutrients and waste in the placenta
\cite{Carmeille.ea2015:Annexin-A5promotesmembraneresealing,Ueki.ea2012:Lossofmaternalannexin}.
...
This study includes only term placentas, and our conclusions are
limited to the set of genes which continue to be expressed at term. We
suspect that many genes which are responsible for establishing shape,
intimacy, and interdigitation are expressed and function at earlier
time points where placental morphology is being established.
Furthermore, while these samples have been dissected to only include
the fetal side, they likely include substantial cell and tissue
heterogeneity. We are therfore not yet able to assign specific genes
in our core placental transcriptome to specific cell types and
tissues. We believe that samples of placentas from earlier in
gestation and from multiple locations within the placenta and at
multiple will address these limitations.
...
maternal-fetal tolerance and genes responsible
for cell-cell and cell-matrix interactions, such as the annexins and \ac{LGALS1}. These
genes were almost certainly present in the placenta of the most recent
common mammalian ancestor of the species included in this study. We hypothesize that
modifications to the expression or function of genes in the core placenta transcriptome
will compromise the
...
multiple members of diverse clades and different time points are still
...
%\begin{backmatter}
  ...
  work, as well as the suggestions of the anonymous reviewers which
  improved this paper and the advice of Monica Uddin in preparing this
  manuscript.
