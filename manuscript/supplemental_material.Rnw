...
print(xtable(placental.classification,
             display=c("s","s","s","s","s"),
             align=c("c","c","c","c","c")),
...
placenta_class_table <- function(class="shape") {
    subtable <- placenta.classification.p[,c("rn",class,paste0(class,".fdr")),with=FALSE]
    setnames(subtable,c("Gene","p","FDR"))
    subtable <- head(subtable[FDR <= 0.05,][order(p),],n=50)
...
\begin{table}
<<pathway_analysis,echo=FALSE,results="asis">>=
pathway <- list()
...
\begin{table}
<<pathway_analysis_class,echo=FALSE,results="asis">>=
pathway <- list()
for (class in c("shape","interdigitation","barrier")) {
    for (database in c("kegg","reactome","biocarta","netpath")) {
...
\caption[Placenta Classification Pathway Enrichment]{Pathways with
  significant and suggestive enrichment ($\text{FDR}\le 0.1$) in genes
