...
top.genes.table <- function(species.name,n=50) {
    ...
    temp <- 
        combined.fpkm[species==species.name &
                      ...
                      !grepl("((RNA\\d|MT)-|^RP[SL]P?\\d+[ABCD]?$)",human_name) &
                      !grepl("((RNA\\d|MT)-|^RP[SL]P?\\d+[ABCD]?$)",human_alignment_symbol) &
                      !grepl("(^[Mm][Tt]-|^R[pP][lsLS]\\d+[abcdABCD]?[l]?(?:-.+)?$|rRNA$)",
                             gene_short_name)
...
gene.expression <- function(species.name,gene,complete=FALSE,units=TRUE) {
    ...
    if (complete) {
        if (any(grepl("^ENS.+G\\d+$",gene))) {
            fpkm <- paste0("\\mathrm{",gene,"} = ",
                           fpkm)
        } else {
            fpkm <- paste0("\\mathit{",gene,"} = ",
                           fpkm)
        }
    }
    if (units || complete) {
...
genes in Supplemental \cref{tab:all_equus_caballus_placenta}.
