...
for (this.species in go.results[grepl("100 No Housekeeping",type) & FDR <= 0.05,unique(species)]) {
    print(xtable(go.results[grepl("100 No Housekeeping",type) & FDR <= 0.05 & species == this.species,
                            list(GO.ID,Term,FDR,Ontology,type)
                            ][order(FDR)],
                 caption=paste0("Signficantly enriched ($\\mathrm{FDR}\\le0.05$) GO terms in ",
