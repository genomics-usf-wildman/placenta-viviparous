...
\begin{abstract}
  \textbf{Introduction} The placenta is arguably the most
  ...
  also identified genes which are expressed in all placentas,
  including genes with immune-modulating properties (\textit{ANXA2},
  \textit{ANXA1}, \textit{S100A11}, and \textit{S100A10}) and
  cell-cell interactions (\textit{LAMC1}, \textit{LUM}, and
  \textit{LGALS1}).

  \textbf{Discussion}

  Still, using the strictest filtering strategy
