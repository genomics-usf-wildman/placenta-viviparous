...
<<load.data,echo=FALSE,results="hide",cache.extra=tools::md5sum(paste0("../data/",c("combined_fpkm","gene_trees","housekeeping_genes_superset","placenta_core_transcriptome","placenta_go_results_bp","palcenta_go_results_mf","hugo_nomenclature")))>>=
...
\begin{table}
  \begin{tabular}{c c p{3in}}
    \toprule
    Species                       & Experiment Number & Sample Number(s) \\
    \midrule
    \textit{Ateles fusciceps}     &  & \\
    \textit{Bos taurus}           &  & \\
    \textit{Canis familiaris}     &  & \\
    \textit{Dasypus novemcinctus} &  & \\
    \textit{Equus caballus}       &  & SRR850299,  SRR850300 \\
    \textit{Loxodonta africana}   &  & \\
    \textit{Homo sapiens}         & GSE66622 & SRR1850945,
                                               SRR1850946, 
                                               SRR1850951, 
                                               SRR1850952, 
                                               SRR1850953, 
    ...
    \textit{Ovis aries}           &  & ERR489144, ERR489145\\
    \textit{Pan paniscus}         &  & \\
    \textit{Spalax carmeli}       &  & \\
    \textit{Sus scrofa}           &           & SRR543893 \\
    \bottomrule
  \end{tabular}
...
\subsection{Top 5\% Transcripts Enrichment}
...
for (this.species in names(species.names)) {
    ...
    this.caption <- paste0("Signficantly enriched ($\\mathrm{FDR}\\le0.05$) GO terms in ",
                           " the top 5\\% of \\textit{",
                           capfirst(this.species),"} expressed transcripts.")
    print(xtable(go.results[grepl("Top 5% No Housekeeping",type) & FDR <= 0.05 & species == this.species,
                            list(GO.ID,Term,FDR,Ontology)
                            ][order(FDR)],
                 caption=this.caption,
                 label=paste0("tab:go_terms_enriched_top_five_",gsub(" ","_",this.species)),
                 display=c("s","s","s","e","s")
                 ),
          size="\\small",
          booktabs=TRUE, 
...
for (this.species in names(species.names)) {
    this.caption <- paste0("Expression of all genes with FPKM $\\ge 50$ in \\textit{",
                           capfirst(this.species),"}")
