...
Single-copy (1-to-1) orthologs in were determined using Ensembl v.80
gene trees via Biomart using the biomaRt R package
\cite{Durinck.ea2009:Mappingidentifiersforintegration}. %
A total of \Sexpr{combined.fpkm[species!="homo sapiens" & mean_fpkm >=
  1,length(unique(human_name))]} single-copy orthologous genes were
expressed in at least one species. We also obtained orthology from the
perspective of each species to all other species using biomaRt package
and Ensembl v.80 database to determine the percentage of highly
expressed genes which had an orthology of one-to-one, one-to-many,
many-to-many, or novel/unknown.

Significant shifts in expression levels between groups of species were
calculated using Student's t test on single-copy orthologs using the mean of
...
were single-copy orthologs with all other analyzed species, with the
...
\caption{Core Placenta Transcriptome.
  \Sexpr{core.placenta.transcriptome.long[housekeeping==FALSE,length(unique(human_name))]}
  single-copy \human ortholog genes expressed (\ac{FPKM}$ \ge 10$) in the
...
\caption{Distribution shape of placenta core genes for all possible
  ...
  shows the number of single-copy orthologous genes included in the core
  ...
  %
  genes would be included if the expression of a set of single-copy orthologs of all 13 Eutherians must be at
  least 10 FPKM, %
  \Sexpr{cpt.distribution.shape[`Expression (FPKM)` > 10 
    & `Min. Exp. in` == '13 of 14 Species',max(num_genes)]}
  %
  single-copy orthologs would be included if 13 of 14 species have an
...
the placenta transcriptome if a single-copy ortholog of that gene was
...
transcriptome by only requiring 10 of 14 species to express single-copy
...
of single-copy orthologs (where the mean expression for genes in the
tree which are present must be $\text{FPKM} \ge 10$), we find
