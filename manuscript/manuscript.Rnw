...
top.genes.table <- function(species.name,n=50) {
    ...
    temp <- 
        hugo.nomenclature[temp][order(-mean_fpkm),
                                list(name_or_id,`Approved Symbol`,`Approved Name`,mean_fpkm)]
    ...
    if (species.name=="homo sapiens") {
        temp[,name_or_id:=paste0("\\href{http://www.ensembl.org/Homo_sapiens/Gene/Summary?g=",name_or_id,"}{",xtable.sanitize.text(name_or_id),"}")]
    } else {
        temp[,name_or_id:=xtable.sanitize.text(name_or_id)]
    }
...
human placenta (\ac{FPKM}=\Sexpr{gene.expression("homo sapiens","PRG2")}; Supplemental \cref{tab:all_homo_sapiens_placenta}), although at
comparatively much lower levels than the \bonobo. A \bonobo gene closely
related to the chorionic somatomammotropins (ENSPTRG00000034560) is also
expressed at high levels %
%
(\ac{FPKM}=\Sexpr{gene.expression("pan paniscus","ENSPTRG00000034560")}); %
%
\cref{tab:top50_bonobo_placenta}). Two other Pan genes included within
this family (ENSPTRG00000030805 which shows similarity to GH2 and
ENSPTRG00000034560) show expression levels of \ac{FPKM}>89. Six
pregnancy-specific beta-1-glycoprotein genes were expressed in the
\bonobo placenta with \ac{FPKM} ranging from 20.885-167.961
(Supplemental \cref{tab:all_pan_paniscus_placenta}). Placental
expression of the genes involved in estrogen and progesterone
production (CYP19A1, HSD3B1 respectively) show lower expression in the
bonobo as compared to humans %
...
\cref{tab:top50_human_placenta}) although the expression is fairly similar to that of 
. In addition, \textit{CGA} is
expressed in the bonobo placenta
...
genes are expressed in \bonobo, one related to CGB and another related
to CGB7; both are not highly expressed (Table 8). Among the other top
50 expressed genes, there is high expression of ribosomal proteins
(e.g., RPL18, RPL23), keratins (e.g., KRT7, KRT19), heat-shock
proteins (e.g., HSPA5, HSPA1A), serpins (SERPINE1, SERPINE2), insulin
growth factor 2 (IGF2) and the hormone adrenomedullin (ADM).
