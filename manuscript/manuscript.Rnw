...
\begin{abstract}
  ...
  the pre-eclampsia associated genes \ac{KISS1}, \ac{CRH},
  \ac{PAPPA}, and \ac{ADAM12} on the \human lineage.
...
nor was \ac{MFSD2A}, the Syncytin-2 receptor (\ac{FPKM} $\ge 10$
...
which forms tight junctions in epithelia, the proteglycans \ac{SDC4} and
\ac{LUM}, and \ac{LAMC1}.
...
attachment include \ac{EREG}, a ligand of EGFR, \ac{UNC93A}, a
homolog of \ac{UNC93B1} which is involved in the balancing between
...
of \ac{ERVFRD-1} and \ac{ERVW-1} are widely expressed in most
...
placenta. \ac{ANXA5} is known to be expressed at multiple stages
...
proximity to start the fusion process, we hypothesize that \ac{ANXA5} is
...
core placenta transcriptome (including \ac{GRB2}, \ac{RALB})
as well as annexins which are involved in the location of EGF receptor
localization (\ac{ANXA1} and \ac{ANXA2}) in the core placenta
...
Another component of the core placenta transcriptome, \ac{CD9},
