...
for (this.species in go.results[grepl("100 No Housekeeping",type) & FDR <= 0.0005,unique(species)]) {
    print(xtable(go.results[grepl("100 No Housekeeping",type) & FDR <= 0.0005 & species == this.species,
                            list(GO.ID,Term,FDR,Ontology,type)
                            ],
                 caption=paste0("Signficantly enriched ($\\mathrm{FDR}\\le0.0005$) GO terms in ",
                     "\\textit{",capfirst(this.species),"}"),
                 label=paste0("tab:go_terms_enriched_",gsub(" ","_",this.species)),
                 display=c("s","s","s","e","s","s")
                 ),
          size="\\small",
          booktabs=TRUE,
          floating=TRUE)
...
for (this.species in combined.fpkm[,sort(unique(species))]) {
    print(xtable(genes.expression.table(species.name=this.species),
                 caption=paste0("Expression of all genes with FPKM $\\ge 50$ in \\textit{",
