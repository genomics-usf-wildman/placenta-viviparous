...
print(ggplot(star.logs[grepl("%",field),],
             aes(x=species,y=as.numeric(gsub("%","",value)),
                 color=field))
      + geom_boxplot()
...
@ 
\caption{Transcript statistics for each species}
...
top.genes.table <- function(species.name,n=50) {
    temp <- combined.fpkm[species==species.name &
                      !grepl("^MT-?",gene_short_name) & 
                          !grepl("(RNA\\d|MT)-",human_name)
                          ][order(-mean_fpkm)][!duplicated(gene_short_name)][1:n,list(gene_short_name,human_name,mean_fpkm)]
