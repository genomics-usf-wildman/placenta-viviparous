...
combined.fpkm.wide <- 
    data.table(dcast(combined.fpkm,
                     human_alignment_symbol~species,
                     fun.aggregate=sum,
                     value.var="mean_fpkm"))
...
setkey(combined.fpkm.wide,"human_alignment_symbol")
...
core.placenta.transcriptome.long <- 
    data.table(melt(combined.fpkm.wide[all_expression >= 10,
                                         ...
                                         ][!(human_alignment_symbol %in% 
                                                 housekeeping.genes.superset[,gene_short_name])],
                    id.vars="human_alignment_symbol",
                    variable.name="Species",
                    value.name="FPKM"
                    ))
core.placenta.transcriptome.long[,human_name:=
                                     reorder(human_alignment_symbol,
                                             combined.fpkm.wide[core.placenta.transcriptome.long[,human_alignment_symbol],median_expression])]


core.placenta.transcriptome.genes.shape <-
    combined.fpkm.wide[,list(human_alignment_symbol,
                             median_expression,
                             all_expression,
...
all.genes <- combined.fpkm.wide[!is.na(human_alignment_symbol),unique(human_alignment_symbol)]
placenta.transcriptome.genes <- 
    core.placenta.transcriptome.long[!is.na(human_alignment_symbol),unique(human_alignment_symbol)]
...
pts <- pts[,placenta.ts.permissive:=all.genes %in%
                core.placenta.transcriptome.genes.shape[all_but_four_expression > 10 &
                                                        ! is.na(human_alignment_symbol),
                                                        unique(human_alignment_symbol)]
           ]
