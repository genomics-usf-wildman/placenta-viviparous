...
Following this, reads were trimmed based on quality in Galaxy using
the FASTQ Trimmer v. 1.0.0
\cite{Blankenberg.ea2010:ManipulationofFASTQdata}. All transcriptomes
were then aligned to reference genomes and assembled using the Tuxedo
suite of next-generation sequencing analysis tools. Annotated
reference genomes were downloaded from Ensembl v. 64
\cite{Flicek.ea2012:Ensembl2012}, including \textit{Homo sapiens}
GRCh37, \textit{Pan troglodytes} CHIMP2.1 (6X coverage), \textit{Canis
  familiaris} BROADD2 (7.6X), \textit{Bos taurus} UMD3.1 (9.5X),
\textit{Loxodonta africana} loxAfr3 (7X), and \textit{Monodelphis
  domestica} BROADO5 (6.8X). The genome of the closely-related Pan
troglodytes was used to map the transcriptome of Pan paniscus; the
separation between the two recognized Pan species is generally
perceived to be ~1 Mya (Prufer et al., 2012). Reads were aligned to
reference genomes using Bowtie v. 0.12.7\cite{Langmead2009} and TopHat
v. 1.3.3 \cite{Trapnell.ea2009:TopHatdiscoveringsplicejunctions}.
Cufflinks v. 1.2.1 was then used to assemble transcripts and quantify
expression values using fragments per kilobase of exon per million
mapped reads (FPKM)
\cite{Trapnell.ea2010:Transcriptassemblyandquantification}. RNA-Seq
metrics were computed using Picard
(\url{http://picard.sourceforge.net/}). For comparison, we also
downloaded the placenta Illumina RNA-Seq transcriptome of a mouse from
\cite{Wang.ea2011:surveyfornovelimprinted}. Raw reads were mapped as
above using the Ensembl v. 64 annotation of the \textit{Mus musculus} (house
mouse) genome (NCBI m37).
...
(interpreted as “pull” toward the optimum). In addition, the OU model
