...
<<core.placenta.transcriptome.distribution,results="hide",echo=FALSE,message=FALSE>>=
...
cpt.distribution.shape <- 
    lapply(c("all_expression",
           ...
           function(x){
               calculate.distribution.shape(cptg.nh[,x,with=FALSE][[1]])[,`Minimum Expression in`:=x]
           })
fix.expression.names <- function(x){
    ...
    nc.levels <- paste0(num.species," of ",num.species," species")
    ## number coutner
...
cpt.distribution.shape <- 
    cpt.distribution.shape[,`Minimum Expression in`:=
...
`Minimum Expression in` == '14 of 14 species',max(num_genes)]} %
...
\subsubsection{Degree of Intimacy}

\subsubsection{Interdigitation}
