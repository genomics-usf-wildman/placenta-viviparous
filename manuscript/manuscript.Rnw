...
go.results <- rbindlist(list(placenta_go_results_mf[1:10,],
                             placenta_go_results_bp[1:10,]))
go.results <- go.results[,Annotated:=NULL]
setnames(go.results,"classic","p value")
setnames(go.results,"classic.fdr","FDR")
setnames(go.results,"GO.ID","GO ID")
setnames(go.results,"Term","GO Term")
print(xtable(go.results,
             digits=2,
