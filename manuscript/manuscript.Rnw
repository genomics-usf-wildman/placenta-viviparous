...
\Sexpr{combined.fpkm[species!="homo sapiens" & mean_fpkm >=
  1,length(unique(human_name))]} 1:1 orthologous genes were
expressed in at least one species. Of these,
\Sexpr{combined.fpkm[species!="homo sapiens" & mean_fpkm >=
  10,length(unique(human_name))]} 1:1 orthologous genes with
\ac{FPKM} $\ge 10$ were retained for future analyses of gene
expression evolution described below.
...
expression %
(\ac{FPKM}=\Sexpr{gene.expression("canis
  familiaris","ENSCAFG00000002115")}) %
...
HTRA1). The gene \textit{LGALS1}, which shows evidence of
immunosupression during pregnancy
\cite{Than.ea2008:Emergenceofhormonaland}, shows high expression in
dog %
(\ac{FPKM}=\Sexpr{gene.expression("canis familiaris","LGALS1")}). %
In addition, the gene SSLP1, a gene expressed in the placentas of cow
and sheep but not present in primates (Ushizawa et al., 2009), is also
