...
\begin{frame}{Makefile rules}
\centering
\includegraphics[keepaspectratio,height=0.8\textheight]{figures/default}
\end{frame}

\begin{frame}[fragile]{Common Options}
\tiny
\inputminted[firstline=8, lastline=36]{make}{../fastq/common_makefile}
\end{frame}

\begin{frame}[fragile]{FASTQC}
\inputminted[firstline=78, lastline=83]{make}{../fastq/common_makefile}
\end{frame}

\begin{frame}[fragile]{Trinity}
\tiny
\inputminted[firstline=211, lastline=222]{make}{../fastq/common_makefile}
\end{frame}

\begin{frame}[fragile]{Diamond}
\tiny
\inputminted[firstline=249, lastline=268]{make}{../fastq/common_makefile}
\end{frame}


\begin{frame}[fragile]{STAR Indexes}
\tiny
\inputminted[firstline=86, lastline=105]{make}{../fastq/common_makefile}
\end{frame}

\begin{frame}[fragile]{STAR}
\tiny
\inputminted[firstline=106, lastline=118]{make}{../fastq/common_makefile}
\end{frame}

\begin{frame}[fragile]{Cufflinks}
\tiny
\inputminted[firstline=43, lastline=60]{make}{../fastq/common_makefile}
\end{frame}
...
\begin{figure}
<<transcriptome.mapping.figure,fig.width=4,fig.height=4,echo=FALSE>>=
...
\begin{figure}
<<transcriptome.mapping.figure.unmapped,fig.width=4,fig.height=4,echo=FALSE>>=
star.logs.unmapped <- 
...
\begin{figure}
<<core.placenta.transcriptome,fig.width=10,fig.height=6,out.height="0.8\\textheight",echo=FALSE>>=
print(ggplot(core.placenta.transcriptome.long[!grepl("(min|median|max)_expression",Species),],
             aes(x=human_name,y=gsub("^([^ ])[^ ]* +([^ ]*)","\\1. \\2",capfirst(Species))))
...
\column{0.6\textwidth}
\begin{figure}
<<hormone.genes.plot,echo=FALSE,warning=FALSE,message=FALSE>>=
