...
@ 
\caption{Expression in \ac{FPKM} of top 50 genes in \horse which are
  not ribosomal, mitochondrial, or whose human ortholog is a
  housekeeping gene}
...
The top 50 expressed genes in the term placenta of \armadillo are in
\cref{tab:top50_armadillo_placenta}.
