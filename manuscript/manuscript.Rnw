...
\begin{abstract}
  ...
  \Sexpr{star.logs[,length(unique(species))]} species of mammals, and
  compared the evolution of expression across clades.
  ...
  invasion (\textit{GRB2} and \textit{RALB5}) and trophoblast fusion
  (\textit{ANXA5} and \textit{ANXA1}). %
  ...
  \textbf{Discussion} %
  Using the strictest filtering strategy we find a core set of only
  \Sexpr{core.placenta.transcriptome.long[housekeeping==FALSE,length(unique(human_name))]}
  genes is essential for the proper functioning of the placenta in all
  species analyzed. We also found significantly higher expression of
  the pre-eclampsia associated genes \textit{KISS1}, \textit{CRH},
  \textit{PAPPA}, and \textit{ADAM12} on the \human lineage.
...
of pregnancy
\cite{Carter2012:Evolutionofplacentalfunction,Knox.ea2008:Genomicevolutionofplacenta,Su.ea2002:Large-scaleanalysisofhuman}.
...
\cite{Green.ea2000:Pregnancy-associatedbovineandovine}.
...
\textit{CRH} (FDR=\Sexpr{hum.all.lin.spec["CRH",`FDR`]},
...
expressed with $\text{FPKM} \ge 10$ in all species studied and was not
a human housekeeping gene according to
...
\citeC{arter.ea2011:Evolutionofinvasiveplacentation}.
