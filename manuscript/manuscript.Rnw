...
intricate “arms race”, involving the imprinting of parental alleles,
...
sister species to the chimpanzee), \dog, \cow, \elephant, and
...
short-tailed opossum], \gallili [?], \carmelli [?]). Information on
...
\DLA{citation; e-mailed Alec Archibald from the ISGC about what the
  right one should be}, and \pig
...
which would be included in the core placenta set if the cutoff was set
differently.
%
\textit{ANXA1} is
involved in the modulation of the inflammatory response \DLA{It does
  more stuff; read the huthcinson2011 review and fill this in}
%
