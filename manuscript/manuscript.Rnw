...
\begin{abstract}
  ...
  core genes which are expressed (FPKM $\ge 10$) in all placentas,

  including genes with immune-modulating properties (\textit{ANXA2},
  \textit{ANXA1}, \textit{S100A11}, and \textit{S100A10}), cell-cell
  interactions (\textit{LAMC1}, \textit{LUM}, and \textit{LGALS1}),
  and trophoblast fusion (\textit{ANXA5} and \textit{ANXA1}).
...
mammals, it varies widely in terms of the shape of attachment, degree
...
as the length of gestation; these traits are often with specific
aspects of placental morphology
...
genes which are responsible for the \emph{core functionality of the
  placenta} and 2) which genes are associated with \emph{changes in
  placenta morphology}?

Previous studies of some species (i.e., \textit{Homo sapiens},
\textit{Mus musculus}, \textit{Bos taurus}) have provided some
preliminary answers to these questions. They revealed a stunning
...
\textit{Spalax carmeli}, \textit{Spalax galili}, and \cow, \acp{PSG}
in \human and \mouse, \acp{PAG} in \cow, genes related to the
thymus-specific serine protease \textit{PRSS16} in \textit{Loxodona
  africana} (in this work), and the expansion of the growth hormone
...
In order to address our two questions in a more systematic way, we
sequenced and quantified the total RNA of mammalian placentas at term,
including that of the \human, \textit{Pan paniscus} (the sister
species to \textit{P. troglodytes}), \textit{Ateles fusciceps},
\mouse, \galili, \carmeli, \textit{Canis familiaris}, \cow, \elephant,
\textit{Dasypus novemcinctus}, and \textit{Monodelphis domesticus}.
...
\cite{Coleman.ea2010:Structuralannotationofequine}, \textit{Ovis
  aries} \cite{Archibald.ea2010:sheepgenomereferencesequence}, and
\textit{Sus scrofa} \cite{Ahanda.ea2012:Predictionofaltered3-} to: 1)
identify the non-housekeeping genes which are expressed in the
placentas of all species analyzed and therefore represent the
\emph{core functionality of the placenta}; 2) identify the genes whose
...
\begin{figure}
<<species.tree,fig.width=8,fig.height=6,out.width="\\columnwidth",echo=FALSE,message=FALSE,warning=FALSE>>=
species.tree <- 
...
Placenta morphology with a defined order (interdigitation, intimacy)
were additionally analyzed using proportional odds logistic regression
using the MASS package in R~\cite{Venables2002:MASS}.


\subsection{Gene Set and Pathway Enrichment Analysis}

Gene sets and pathways in were obtained from MSigdb v5.1 hallmark gene
set \cite{Liberzon.ea2011:MolecularsignaturesdatabaseMSigDB} and Gene
Ontology which represent a superset of available pathway databases,
including Reactome, Kegg, Netpath, and GO. We calculated the
over-representation of gene lists in these gene sets using Fisher's
exact test and corrected for multiple testing by estimating the
\ac{FDR} using the Benjamini and Hochberg procedure
...
predictor of unmapped read percentage (Supplemental
...
We performed \ac{gsea} on both the top 1\% of transcripts
(Supplemental \cref{tab:go_terms_enriched_top_one_ateles_fusciceps,
  tab:go_terms_enriched_top_one_homo_sapiens,
  tab:go_terms_enriched_top_one_pan_paniscus,
  tab:go_terms_enriched_top_one_mus_musculus,
  tab:go_terms_enriched_top_one_spalax_galili,
  tab:go_terms_enriched_top_one_spalax_carmeli,
  tab:go_terms_enriched_top_one_bos_taurus,
  tab:go_terms_enriched_top_one_ovis_aries,
  tab:go_terms_enriched_top_one_sus_scrofa,
  tab:go_terms_enriched_top_one_equus_caballus,
  tab:go_terms_enriched_top_one_canis_familiaris,
  tab:go_terms_enriched_top_one_loxodonta_africana,
  tab:go_terms_enriched_top_one_dasypus_novemcinctus,
  tab:go_terms_enriched_top_one_monodelphis_domestica}) and the top
100 transcripts (Supplemental
\cref{tab:go_terms_enriched_ateles_fusciceps,
  tab:go_terms_enriched_homo_sapiens,
  tab:go_terms_enriched_pan_paniscus,
  tab:go_terms_enriched_mus_musculus,
  tab:go_terms_enriched_spalax_galili,
  tab:go_terms_enriched_spalax_carmeli,
  tab:go_terms_enriched_bos_taurus, tab:go_terms_enriched_ovis_aries,
  tab:go_terms_enriched_sus_scrofa,
  tab:go_terms_enriched_equus_caballus,
  tab:go_terms_enriched_canis_familiaris,
  tab:go_terms_enriched_loxodonta_africana,
  tab:go_terms_enriched_dasypus_novemcinctus,
  tab:go_terms_enriched_monodelphis_domestica}); common processes
...
\begin{figure}[tbh]
<<core.placenta.transcriptome,fig.width=5,fig.height=8,out.height="0.8\\textheight",echo=FALSE>>=
cptl <-
...
print(ggplot(cptl,aes(x=human_name,y=FPKM))+
...
\begin{figure}
<<core.placenta.transcriptome.distribution,fig.width=5,fig.height=5,out.height="0.8\\textheight",echo=FALSE,message=FALSE>>=
...
fix.expression.names <- function(x){
    x <- gsub("^all_expression$",
              paste0(num.species," of ",num.species),
              x
              )
    nc.levels <- paste0(num.species," of ",num.species)
    ## number coutner
    nc <- c("one"=1,"two"=2,"three"=3,"four"=4)
    for (a in names(nc)) {
        x <- gsub(paste0("^all_but_",a,"_expression$"),
              paste0(num.species-nc[a]," of ",num.species),
              x
              )
        nc.levels <-
            c(nc.levels,
...
print(ggplot(cpt.distribution.shape[`Expression (FPKM)` > 0.1,],
      ...
      + ylab("Number of Genes Meeting FPKM Cutoff in N species")
      ...
      + guides(color=guide_legend(title=paste0("Min. Expression in\nn of ",num.species," Species"),
                                  keywidth=unit(3,"mm"),
                                  keyheight=unit(3,"mm"),
                                  title.hjust=0.5,
...
\caption{Distribution shape of placenta core genes. The vertical axis
  ...
  \Sexpr{cpt.distribution.shape[`Expression (FPKM)` > 10 
    & `Min. Exp. in` == '14 of 14',max(num_genes)]}
  %
  genes would be included if the FPKM of a set of 1:1 orthologs of all 14 species must be at
  least 10, %
  \Sexpr{cpt.distribution.shape[`Expression (FPKM)` > 10 
    & `Min. Exp. in` == '13 of 14',max(num_genes)]}
  %
  1:1 orthologs would be included if 13 of 14 species have an
   with an FPKM of at least 10, and 
  \Sexpr{cpt.distribution.shape[`Expression (FPKM)` > 10 
    & `Min. Exp. in` == '10 of 14',max(num_genes)]}
...
cutoff was set differently; for example, choosing an \ac{FPKM} cutoff of 1 yields %
\Sexpr{cpt.distribution.shape[`Expression (FPKM)`>=1 & 
`Min. Exp. in` == '14 of 14',max(num_genes)]} %
...
\textit{ANXA5}, which was originally identified as a an anti-coagulant
protein, but is also involved in syncitium formation, membrane repair,
and reduction of spontaneous
abortion~\cite{Ueki.ea2012:Lossofmaternalannexin}. Other notable
syncytiotrophoblast fusion proteins, Syncytin-1 (\textit{ERVW-1}) and -2
(\textit{ERVFRD-1}) were not present in the core placenta, as they (or
their homologs) are not highly expressed in \dog, \bonobo, or \sheep,
nor was \textit{MFSD2A}, the Syncytin-2 receptor (\ac{FPKM} $\ge 10$
...
pathways related to translation (ribosome, translation, signal
recognition particle targeting to the rough endoplasamic reticulum,
...
into the endometrium, including the EGFR1 pathway and the Actin Y
pathway. Two other significant pathways, the mPR/Progesterone pathway
...
\cref{fig:kegg_actin}, which shows that \textit{FN1} and \textit{ITG}
are highly expressed, which activate Rac, and potentially lead to the
formation of focal adhesions and lamellipodia formation through their
inhibition of GSN and HSPC300. Other components of cell-cell
adhesion, including \textit{F11R} which forms tight junctions in
epithelia, proteglycans \textit{SDC4} and \textit{LUM}, and the
laminin \textit{LAMC1}.
...
\subsubsection{Placenta Attachment Shape}

Supplemental \cref{tab:shape_genes} shows the top 50 genes which are
associated with the shape of placenta attachment, as well as the shape
in which the difference is most significant. The most significant
genes which were associated with diffuse placenta attachment include
\textit{EREG}, a ligand of EGFR, \textit{UNC93A}, a homolog of
\textit{UNC93B1} which is involved in the balancing between Tlr7 and
Tlr9 immune response in
mice~\cite{Brinkmann.ea2007:interactionbetweenERmembrane}, and
\textit{WNT5A}, which is a member of the Wnt ligand family whose
singaling pathway has been implicated in motility in human trophoblast
...
which were associated with different levels of interdigitiation and
...
suspect this is due to the small number of intermediate
interdigitation and intimacy placentas that are available. Future
analyses of additional placentas with intermediate interdigitation and
intimacy should better illuminate the pathways which are responsible
for these placental morphologies.
...
While Syncitin-1 and -2 are implicated in the formation of the
syncytiotrophoblast in
\human~\cite{Rawn.ea2008:evolutionregulationandfunction} and homologs
of \textit{ERVFRD-1} and \textit{ERVW-1} are widely expressed in most
of the placentas analyzed here, they are not expressed in \bonobo,
\sheep, and \dog, even though these mammals form
...
syncytiotrophoblast is likely to be highly expressed in the core
placenta. \textit{ANXA5} is known to be expressed at multiple stages
of placentation, and is capable of repairing membrane defects and
promoting the fusion of
cells~\cite{Carmeille.ea2015:Annexin-A5promotesmembraneresealing} in
the presence of Ca$^{2+}$. While there are likely other genes which
are responsible for the positioning of cytotrophoblast cells in close
proximity to start the fusion process, \textit{ANXA5} is likely to be
essential for the completion of fusion and the maintenance of membrane
repair during the massive flux of nutrients and waste in the placenta.
...
The high expression of components of the EGFR signaling pathway in the
core placenta transcriptome (including \textit{GRB2}, \textit{RALB})
as well as annexins which are involved in the location of EGF receptor
localization (\textit{ANXA1} and \textit{ANXA2}) in the core placenta
is consistent with previous findingsshowing that alterations to the
EGF signaling system change invasion of trophoblast cells into the
spiral arteries~\cite{Armant.ea2015:Reducedexpressionofepidermal}.
Another component of the core placenta transcriptome, \textit{CD9},
has been shown to suppress \human extravillous trophoblast
