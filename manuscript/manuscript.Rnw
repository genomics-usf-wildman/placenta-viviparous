...
housekeeping genes is to use the tissue specificity
index~\cite{Yanai.ea2005:Genome-widemidrangetranscriptionprofiles}.
Supplemental \cref{tab:tissue_specificity_index_of_plaenta_core_genes}
lists the tissue specificity index for all housekeeping and
non-housekeeping genes; housekeeping genes expressed in all placentas
have a lesser median tissue specificity index than non-housekeeping
genes (Supplemental
\cref{fig:tissue_specificity_index_with_housekeeping}).
