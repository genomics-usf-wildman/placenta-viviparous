...
for (mf.bp in c("mf","bp")) {
    for (class in c("interdigitation","barrier","shape")) {
...
sequenced and quantified the mRNA environment of mammalian placentas
at term, including that of the \human, \textit{Pan paniscus} (the
sister species to \textit{P. troglodytes}), \textit{Ateles fusciceps},
