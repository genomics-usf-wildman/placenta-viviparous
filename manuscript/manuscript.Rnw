...
\newcommand{\smonkey}[0]{\textit{A. fusciceps}}
\newcommand{\cow}[0]{\textit{B. taurus}}
\newcommand{\dog}[0]{\textit{C. familiaris}}
\newcommand{\armadillo}[0]{\textit{D. novemcinctus}}
\newcommand{\horse}[0]{\textit{E. galili}}
\newcommand{\elephant}[0]{\textit{L. africana}}
\newcommand{\human}[0]{\textit{H. sapiens}}
\newcommand{\opossum}[0]{\textit{M. domestica}}
\newcommand{\mouse}[0]{\textit{M. musculus}}
\newcommand{\galili}[0]{\textit{N. galili}}
\newcommand{\sheep}[0]{\textit{O. aries}}
\newcommand{\bonobo}[0]{\textit{P. paniscus}}
\newcommand{\carmeli}[0]{\textit{S. carmeli}}
\newcommand{\boar}[0]{\textit{S. scrofa}}
...
in the term placenta of \textit{Canis familiaris} are shown in
...
In addition, the gene \textit{SSLP1}, a gene expressed in the
placentas of \textit{Bos tarus} (%
\Sexpr{gene.expression("bos tarus","SSLP1")}, %
\cref{tab:top50_cow_placenta}) and \textit{Ovis Aries}
(\Sexpr{gene.expression("ovis aries","SSLP1")},
\cref{tab:top50_sheep_placenta}) but is not present in primates
\cite{Ushizawa.ea2009:Characterizationandexpressionanalysis}, is also
expressed at high levels
%
(\ac{FPKM}=\Sexpr{gene.expression("canis familiaris","SSLP1")}). %
There is no placental expression of the estrogen synthase gene
(CYP19A1) in dogs, however expression of HSD3B1 is relatively high
...
tree.id <-
    genes.to.tree[combined.fpkm[gene_short_name=="SSLP1" & species=="homo sapiens",gene_id],tree]
gene.tree.table <- trees[[tree.id]]$table
gene.tree.expression <- combined.fpkm[gene_id %in% gene.tree.table[,gene_id]]
gene.tree <- read.tree(text=trees[[tree.id]]$tree.dat)
plot.tree.matrix(read.tree(text=trees[[tree.id]]$tree.dat),gene.tree.table,gene.tree.expression)
