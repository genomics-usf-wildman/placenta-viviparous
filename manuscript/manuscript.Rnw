...
<<load.data,echo=FALSE,results="hide",cache.extra=tools::md5sum(paste0("../data/",c("combined_fpkm","gene_trees","housekeeping_genes_superset")))>>=
for (a in paste0("../data/",c("combined_fpkm","gene_trees","housekeeping_genes_superset"))) {
    load(a)
}
...
\begin{figure}
<<core.placenta.transcriptome,fig.width=8,fig.height=12,echo=FALSE>>=
combined.fpkm.wide <- 
    data.table(dcast(combined.fpkm,
                     ...
                     value.var="mean_fpkm"))
temp <- 
    combined.fpkm.wide[,median_expression := apply(combined.fpkm.wide[,-1,with=FALSE],1,median)]
temp <- 
    combined.fpkm.wide[,min_expression := apply(combined.fpkm.wide[,-1,with=FALSE],1,min)]
...
temp <- core.placenta.transcriptome.long[,human_name:=
                                     reorder(human_name,
                                             combined.fpkm.wide[core.placenta.transcriptome.long[,human_name],median_expression])]
rm(temp)

print(ggplot(core.placenta.transcriptome.long[!Species %in% c("median_expression","min_expression"),],
      ...
      + theme(legend.position="top",
              ...
              axis.text.x = element_text(angle = 340, hjust = 0.3, vjust=0.8,face="italic"))
      )
                            
# 
# print(xtable(combined.fpkm.wide[min_expression > 10,
#                                 ][order(-median_expression)
#                                   ][!(human_name %in% 
#                                           housekeeping.genes.superset[,gene_short_name])][1:75,],
#              digits=0
#              ),
...
TIMP3 -- maybe in pre-eclampsia (23172037)
SERPINH1? with TIMP3 
% http://www.proteinatlas.org/ENSG00000149257-SERPINH1/tissue/placenta

Why are RPL17 and RPS26 here? They are ribosomal proteins. These
...
\begin{figure}
<<prolactin.evolution.tree,echo=FALSE,fig.width=8,fig.height=8>>=
...
\begin{figure}
<<galectins.evolution.tree,echo=FALSE,fig.width=8,fig.height=8>>=
