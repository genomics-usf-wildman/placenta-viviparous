...
230 base pairs (bp). Read lengths for species are shown in
\cref{tab:ref_genome_aligned_species}.

\subsubsection{Additional Sequences}

For more complete comparisons additional sequences of placenta in
\mouse \cite{Wang.ea2011:surveyfornovelimprinted}, \human
\cite{Hughes.ea2015:Evaluatingintra-andinter-individual} \horse
\DLA{citation}, \sheep \DLA{citation}, and \boar \DLA{citation} were
obtained from the \ac{SRA} (Supplemental \cref{tab:srr_numbers}).
