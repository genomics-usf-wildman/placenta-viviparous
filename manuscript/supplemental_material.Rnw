...
lau.all.lin.spec <- placenta.lineage.specific.table("lau.all",fdr.cutoff=0.5)
...
print(ggplot(cpt.h.distribution.shape[`Expression (FPKM)` > 0.1,],
      ...
      + geom_segment(mapping=aes(y=V1-ifelse(V1<160,40,0),
                         yend=V1,
                         xend=10,
                         x=sapply(V1,
                                  function(x){
...
\begin{figure}[tbh]
<<core_placenta_tree_transcriptome,fig.width=7,fig.height=8,out.height="0.8\\textheight",echo=FALSE>>=
cpttl <-
...
print(ggplot(cpttl[FPKM > 0.1,],aes(x=tree_genes,y=FPKM))+
      geom_boxplot(outlier.shape=NA)+
      scale_y_log10(labels=trans_format("log10", math_format(10^.x)))+
      geom_point(aes(color=Species,shape=Species))+
      scale_shape_manual("Species",
...
\begin{figure}
<<core_placenta_tree_transcriptome_housekeeping,fig.width=9,fig.height=18,out.height="0.8\\textheight",echo=FALSE>>=
cpttlh <-
    core.placenta.tree.transcriptome.long[!grepl("(min|median|max|all)_expression|all_but_",
                                                 Species) & housekeeping==TRUE,]
...
print(ggplot(cpttlh[FPKM > 0.1,],aes(x=tree_genes,y=FPKM))+
      geom_boxplot(outlier.shape=NA)+
      scale_y_log10(labels=trans_format("log10", math_format(10^.x)))+
      geom_point(aes(color=Species,shape=Species))+
      scale_shape_manual("Species",
