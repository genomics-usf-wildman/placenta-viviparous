...
<<source.to.latex,echo=FALSE,results="hide",cache.extra=tools::md5sum(paste0("../common_r_code/",c("to_latex.R")))>>=
for (a in paste0("../common_r_code/",c("to_latex.R"))) {
    source(a)
}
@ 
...
RNA-Seq technology to sequence the transcriptomes of 6 species of
therian (i.e. marsupials and “placental”) mammals. We identified
significant shifts in gene expression along specific evolutionary
lineages using a model of character evolution. Genes that showed
significant elevation in expression along the human lineage
...
Keywords: mammal, gene expression, evolution, RNA-seq, placenta
...
the most variable organ in terms of structure and form (Mossman,
1937). Species differ in the shape of attachment (i.e., discoid,
diffuse, cotyledonary, zonary), degree of invasiveness of fetal
tissues into maternal tissues (i.e. epitheliochorial,
endotheliochorial, hemochorial), and degree of interdigitation of
maternal-fetal tissue (i.e., villous, folded, trabecular, lamellar,
labrynthine) (Mossman, 1987). In addition, therian mammals differ
drastically in the number and size of their offspring, as well as the
length of gestation, traits often associated with specific aspects of
placental morphology (Capellini et al., 2011).
...
(Renfree, 2010). Marsupials also differ from eutherian mammals in
...
fusion of the chorion and allantois (Mossman, 1987). It is unclear
...
separated by one tissue layer, as in humans and mouse (Elliot
and Crespi, 2009; Wildman et al., 2006).

Although the morphology of the mammalian placenta has been well
characterized in a number of species (Mossman, 1987), the diversity of
the molecular environment is only beginning to be understood from a
genetic perspective (Carter, 2012). Studies of certain model species
...
(Carter, 2012; Hou et al., 2012; Knox and Baker, 2008; Su et al.,
2002). For example, each lineage has evolved expansive
placenta-specific gene families with expression patterns that can vary
in space and time (Green et al., 2000; Simmons et al., 2008). These
...
primates (Haig, 2008; Rawn and Cross, 2008). Expression patterns of
...
vary substantially between humans and mice (Su et al., 2002), and the
recruitment of SIGLEC6 to placental expression in humans differs from
both chimpanzees and mice (Brinkman-Van der Linden et al., 2007).
...
strategies during pregnancy (Haig, 1993). The maternal strategy is to
...
expression at the maternal-fetal interface (Haig, 1993).

One symptom
of this maternal-fetal conflict is its potential affect on human
health (Hollegaard et al., 2013). In addition to having an invasive
hemochorial placenta, humans possess a deeply invasive trophoblast
layer that can penetrate the myometrium of the uterus (Carter and
Pijnenborg, 2011). Defects in placental growth are known to be
...
death of the mother and/or infant (citation). Although cases have been
documented in chimpanzees and gorillas (citation), preeclampsia has a
far higher incidence in human pregnancies with as much as 4\% of
pregnancies being affected in the United States (citation).
...
humans (Carter and Pijnenborg, 2011); however, large-scale expression
...
expression (Brawand et al., 2011). Here we use Illumina RNA-Seq
...
gestation (Table 1). We use this data, along with existing RNA-Seq
data from mouse (Wang et al., 2011), to address the major differences
...
(Homo sapiens [human], Pan paniscus [bonobo], Canis familiaris
[domestic dog], Bos taurus [domestic cow], Loxodonta africana [African
savanna elephant], and Monodelphis domestica [gray short-tailed
opossum]). Information on specimen ID, place and time of collection,
and collector is shown for each sample in Supplemental Table 1. In the
case of Pan, Bos, and Loxodonta, the placenta was collected at term
upon expulsion from the mother following delivery. Placental tissue
from Homo sapiens was collected upon delivery via a caesarian section.
Near-term placental tissue from Canis familiaris was collected upon
the death of the mother from a car accident. Placental tissue from
Monodelphis domestica was sampled at fetal stage 32, roughly one day
prior to birth. Only the fetal portion of the placenta was sampled at
the maternal-fetal interface (i.e. villous portion for Homo, Pan,
Canis, and Loxodonta; cotyledon for Bos; diffuse placenta for
Monodelphis). All tissues were collected and stored in RNAlater (Life
...
following the manufacturer’s instructions. RNA was further purified
...
230 base pairs (bp). Bos, Loxodonta, and Monodelphis were sequenced
with read lengths of 75 bp; Canis, Homo, and Pan were sequenced with
read lengths of 150 bp.

\subsection{Transcriptome Assembly and Mapping}

The quality of reads was evaluated using FASTQC v. 0.9.2
(http://www.bioinformatics.babraham.ac.uk/projects/fastqc/). Following
this, reads were trimmed based on quality in Galaxy using the FASTQ
Trimmer v. 1.0.0 (Blankenberg et al., 2010). All transcriptomes were
then aligned to reference genomes and assembled using the Tuxedo suite
of next-generation sequencing analysis tools. Annotated reference
genomes were downloaded from Ensembl v. 64 (Flicek et al., 2012),
including Homo sapiens GRCh37, Pan troglodytes CHIMP2.1 (6X coverage),
Canis familiaris BROADD2 (7.6X), Bos taurus UMD3.1 (9.5X), Loxodonta
africana loxAfr3 (7X), and Monodelphis domestica BROADO5 (6.8X). The
genome of the closely-related Pan troglodytes was used to map the
transcriptome of Pan paniscus; the separation between the two
recognized Pan species is generally perceived to be ~1 Mya (Prufer et
al., 2012). Reads were aligned to reference genomes using Bowtie v.
0.12.7 (Langmead et al., 2009) and TopHat v. 1.3.3 (Trapnell et al.,
2009). Cufflinks v. 1.2.1 was then used to assemble transcripts and
quantify expression values using fragments per kilobase of exon per
million mapped reads (FPKM) (Trapnell et al., 2010). RNA-Seq metrics
were computed using Picard (http://picard.sourceforge.net/). For
comparison, we also downloaded the placenta Illumina RNA-Seq
transcriptome of a mouse from (Wang et al., 2011). Raw reads were
mapped as above using the Ensembl v. 64 annotation of the Mus musculus
(house mouse) genome (NCBI m37).
...
(Huang et al., 2009; Sherman et al., 2007). DAVID ranks functional
annotation clusters according to enrichment score (ES); functional
annotation clusters are considered significantly enriched if ES>1.3,
as this corresponds to p<0.05 (Huang et al., 2009). We executed
...
FPKM and 2) the top 100 genes with the highest FPKM. Due to the
...
via the Biomart online data management tool (Kasprzyk, 2011). A total
of 6882 1:1 orthologous genes were expressed in at least one species.
For further analysis, we eliminated all genes that did not have at
least one species with an FPKM>10. A list of these 4259 1:1
orthologous genes was retained for future analyses of gene expression
evolution described below.

We used the software OUCH (Ornstein–Uhlenbeck for Comparative
Hypotheses) in the R statistical environment to investigate
significant shifts in gene expression levels over time (Butler and
King, 2004). OUCH institutes an Ornstein-Uhlenbeck (OU) model of
evolution, which allows for different optima for quantitative trait
values along user-specified branches of a phylogenetic tree. The OU
model explicitly accounts for change in quantitative trait values by
introducing two parameters into the model of stochastic Brownian
motion of change in trait value along a phylogenetic tree through
time. These parameters include, ?, or an optimum trait value, and ,
or the strength of selection (interpreted as “pull” toward the
optimum). In addition, the OU model can also integrate evolutionary
change by assuming that each user-specified set of braches has its own
separate OU parameters (see Butler and King, 2004).
...
in which expression in at least one species had an FPKM≥10 as
described above. We corrected for multiple testing using the false
discovery rate (FDR) and calculated adjusted p values (q-values) using
the Benjamini-Hochstein correction. We used the time-calibrated
phylogenetic tree of (Meredith et al., 2011) with all species not used
in this analyses pruned.
...
using DAVID (Huang et al., 2009; Sherman et al., 2007). DAVID ranks
functional annotation clusters according to enrichment score (ES);
functional annotation clusters are considered significantly enriched
if ES>1.3, as this corresponds to p<0.05 (Huang et al., 2009). As
...
monkey (Ateles fusciceps) using similar protocols discussed above.
...
For transcriptomes sequenced with 75 bp, (opossum, elephant, and cow),
the number of reads ranged from 3.61x107 to 3.70x107 and number of
bases ranged from 1.43x109 to 1.64x109 with at least 96\% of reads
mapping to the respective reference genome (Table 1). For
transcriptomes sequenced with 150 bp (human, bonobo, and dog), number
of reads ranged from 6.47x107 to 9.27x107 and number of bases ranged
from 5.20x109 to 6.25x109 with at least 99\% of reads mapping to the
respective reference genome. All reads were submitted to the NCBI
Sequence Read Archive (SRA) (Accession Number XXX-YYY).
...
ranged from 4119 (bonobo) to 6506 (human) for transcripts with FPKM>10
and 11689 (dog) to 30733 (human) for transcripts with FPKM>1. The
...
The top 50 expressed genes in each genome analyzed are shown in Tables
2-7, and all genes for each transcriptome are listed in Supplementary
Tables X-XX [These are large Excel files to be included with the
Supplemental Material]. In addition, specific genes including
placental hormones and hormone-related genes, galectins, and
insulin-like growth factor related genes are shown with FPKM values
side to side for all transcriptomes including the reanalysis of mouse
from Wang et al. (2011) in Table 8. Below we describe each
transcriptome separately, specifically concentrating on
highly-expressed genes.

\subsection{Human Placenta Transcriptome}

The top 50 expressed genes as measured by FPKM in the term placenta of
the human are shown in Table 2 with a list of all genes in
Supplemental Table X. The gene with by far the highest FPKM value
(101962) in humans is chorionic somatomammotropin hormone 1 (CSH1),
followed by chorionic somatomammotropin hormone 2 (CSH2), and
chorionic somatomammotropin hormone-like 1 (CSHL1), all three of which
are members of a distinct expansion of the growth hormone gene family
within hominoids. Another member of the growth hormone gene family,
GH2 (FPKM=729.639), is also expressed to a high degree in the human
placenta (Table 2). Ten pregnancy-specific beta-1-glycoprotein genes
are highly-expressed (FPKM>100) in the term placenta of humans, with
PSG4 showing the greatest degree of expression (FPKM= 5201.31; Table
2). These genes are members of a hominoid-specific expansion of the
carcinoembryonic antigen (CEA) family and known to be expressed by the
syncytiotrophoblast (Lei et al., 1992; Rand and Cross, 2008). Genes
involved in the production of estrogen (CYP19A1) and progesterone
(HSD3B1) show exceptionally high expression in the human placenta
(FPKM = 4817.15 and 829.846 respectively), as well as CGA (FPKM=
4793.08), which generates the alpha subnit of multiple human chorionic
gonadotropin hormones (Table 2). The genes producing the beta subunits
of the chorionic gonadotropins (CGB, CGB1, CGB2, CGB5, CGB7, CGB8) are
either not expressed (CGB8) or expressed modestly (FPKM≤20.826; Table
8). Other highly expressed genes in the top 50 include members of the
hemoglobin family (e.g. HBA1, HBB, HBG2, HBA2), the maternally
imprinted long noncoding RNA gene H19, genes coding for proteins
required for growth and/or cell proliferation (ADAM12, KISS1, IGF2,
GPC3, CSF3R), anticoagulation (TFPI2), and glucose transport (SLC2A1)
among others (Table 2, Supplemental Table X)

\subsection{Bonobo Placenta Transcriptome}

The top 50 expressed genes as measured by FPKM in the term placenta of
the bonobo are shown in Table 3 with a list of all genes in
Supplemental Table X. Expression in the bonobo placenta differs
somewhat from that of the human, as evidenced by the makeup of the top
50 genes (Table 3). The highest expression (FPKM=10149.1) in bonobo is
proteoglycan 2, bone marrow (PRG2), which is involved in insulin
growth factor signaling and angiogenesis during pregnancy (Weyer and
Glerup, 2011). PRG2 is also expressed at relatively high levels in the
human placenta (FPKM=250.675; Supplemental Table X), although at
...
expressed at high levels (FPKM=1232.56; Table 3). Two other Pan genes
included within this family (ENSPTRG00000030805 which shows similarity
to GH2 and ENSPTRG00000034560) show expression levels of FPKM>89. Six
pregnancy-specific beta-1-glycoprotein genes were expressed in the
bonobo placenta with FPKM ranging from 20.885-167.961 (Supplemental
...
expression in the bonobo as compared to humans (CYP19A1 FPKM= 48.6877;
HSD3B1 FPKM= 54.5301; Table 8). In addition, CGA is expressed in the
bonobo placenta (FPKM=202.629), but at much lower levels than humans
...
Table X. The gene with the highest expression (FPKM=48399.9) in the
...
(Than et al., 2008), shows high expression in dog (FPKM= 1405.02). In
...
expressed at high levels (FPKM= 1853.98). There is no placental
expression of the estrogen synthase gene (CYP19A1) in dogs, however
expression of HSD3B1 is relatively high (FPKM=157.622).

\subsection{Cow Placenta Transcriptome}

The top 50 expressed genes as measured by FPKM in the term placenta of
the dog are shown in Table 5 with a list of all genes in Supplemental
Table X. The gene with the highest expression (FPKM=12488.8) was the
...
including PAG2 (FPKM=7255.34), PAG10 (2076.42), BT.83780 (1793.72),
PAG11 (962.19), PAG12 (469.751), BT.88769 (408.584), PAG1 (301.61),
PAG4 (186.496), PAG6 (157.348), PAG8 (153.225), and PAG16 (103.573).
In addition, 14 other PAGs were expressed with FPKM<40 (Supplemental
...
addition to 7 other placental prolactins with expression of FPKM<51
...
shows evidence of high expression (FPKM=1001.45). There is some
placental expression of the estrogen synthase gene (CYP19A1) in cows
(75.1346), and comparatively greater expression of HSD3B1 (FPKM=
157.622).
...
(FPKM=33439.5) was the natriuretic peptide precursor B (NPPB). The
gene with the second highest expression (FPKM=20233) was the microRNA
...
also highly expressed in the elephant placenta (FPKM=7164.3). Five
...
produces progesterone HSD3B1 (FPKM=0). Other genes with high
...
Supplemental Table X. The gene with the highest expression
(FPKM=41740.6) was hemoglobin, epsilon (HBE), which is expressed at
...
high degree in the opossum placenta (FPKM=2452.19 and FPKM=1139.25).
In addition, there is high expression (FPKM=5827.37) of the gene
...
expressed in the placenta (FPKM=456.978). The gene with the second
highest expression (FPKM=22854.8) is superoxide dismutase 3,
...
opossum placenta (FPKM=13572.8). The gene WFDC2, a member of the whey
...
expression of relaxin RLN1 (FPKM= 41.3949).
...
using a list of genes with FPKM values included within the top 5\%
...
p>0.05) in the top 5\% of FPKM values in at least five transcriptomes
...
species (Carter, 2012). Expression and synthesis of hormones is
...
hormones in multiple mammalian species (Table 8).
...
lineage (Wallis, 2009). (Yamamoto et al., 2011) noted the presence of
a prolactin in the trophoblast of the elephant placenta using
immunohistological staining and human derived antibodies. A previous
analysis of the elephant placenta also detected the expression of
prolactin (Hou et al., 2012). Here we confirm that the sole prolactin
in the elephant is not only expressed but highly expressed in the
placenta, and likely functions both in the maintenance of the corpus
luteum and as a placental lactogen (Yamamoto et al., 2011). PRL is not
expressed in the opossum, even though there is evidence that it is
expressed in the placenta of another marsupial, the tammar wallaby
(Menzies et al., 2011). This suggests that placental prolactin
expression in marsupials may be as diverse as in eutherian mammals.
Although, PRL is not expressed to any great degree in the placenta
other mammals, we see very high expression of genes which resulted
from duplications of PRL in both the cow (Table X), as well as mouse
(Supplemental Table X). Describe these
...
(Mannik et al., 2010), and it is possible that the extreme expression
...
expression of GH, as well as trophoblast invasion (Alsat et al., 1998;
Lacroix et al., 2005; Schiessl et al., 2007), and it is likely that
...
numerous chorionic gonadotropins with variable beta subunits (Maston
and Ruvolo, 2002). The function of the chorionic gonadotropins is to
prevent the regression of the corpus luteum (Hanson et al., 1971).
Among the six placentas we analyzed here, CGA is only expressed in the
placentas of humans and bonobos (Table 8), which corresponds with the
emergence of the chorionic gonadotropins in anthropoid primates
(Maston and Ruvolo, 2002). However, oddly, CGA is expressed at much
higher levels in both species than the beta subunits with which its
product dimerizes (Table 8). CGA is one of several genes noted by
(Uddin et al., 2008) to have undergone adaptive evolution of the
...
dog and is used in the diagnosis of canine pregnancy (Steinetz et al.,
1989; Tsutsui and Stewart, 1991); however, the source of relaxin
produced by pregnant African elephants remains unclear (Meyer et al.,
2004). The high rise in relaxin in elephants has been suggested to be
a major signal for the initiation of labor (Niemuller et al., 1998).
Here we show that the placenta of elephants show high expression of
relaxin and is likely the predominant source of relaxin at term (Table
8).
...
myometrium of the uterus (Carter and Pijnenborg, 2011). It is
...
is exceedingly rare in these species (Carter and Pijnenborg, 2011). It
...
with invasive hemochorial placentas like bonobos (FPKM= 199.924),
spider monkeys (FPKM= 96.24), and the mouse (FPKM= 80.7545)
...
and like GPC3 it is expressed in the bonobo (FPKM= 46.1553), spider
monkey (FPKM=30.41) and mouse (FPKM=2.25391) as well, although to a
...
(FPKM=17.6394; Supplemental Table X). KISS1 is thought to be involved
