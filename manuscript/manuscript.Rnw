...
in Supplemental \fref{tab:all_human_placenta}. The gene with by far
...
hormone gene family within hominoids (~\ref{fig:galectins_evolution_tree}). Another member of the growth
...
\textit{CGB7}, \textit{CGB8}) are either not expressed (%
%
\Sexpr{gene.expression(species.name="homo sapiens",gene="CGB8")}
%
) have reasonable expression ($\mathrm{FPKM} \ge \Sexpr{cgb.min.expression}$),
and the sum of their expression is on the order of the expression of
expression of \textit{CGA}
($\sum \mathit{CGBs}=\Sexpr{cgb.sum.expression}$ FPKM) .
%
%
