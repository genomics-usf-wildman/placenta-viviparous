...
cpt.distribution.shape <- 
    data.table(melt(
        core.placenta.transcriptome.genes.shape[fifth_min_expression > 0,
                                                list(human_name,
                                                     min_expression,
               ...
               id.vars="human_name",
               variable.name="Lowest Expression",
               value.name="Expression (FPKM)"
               ))
cpt.distribution.shape <- 
...
print(ggplot(cpt.distribution.shape[`Expression (FPKM)` > 0,],
             aes(x=`Expression (FPKM)`+1,color=`Lowest Expression`))
      + geom_histogram(aes(y=..count..+1))
      + xlab(expression(FPKM+1))
      + ylab("Genes with N-th Lowest Expression + 1")
      + scale_y_log10()
      + scale_x_log10())
@ 
\caption{Distribution shape of placenta core genes. A histogram and
  density plot of the first through fifth lowest expression in each of
  \Sexpr{star.logs[,length(unique(species))]} species for each of
  \Sexpr{nrow(core.placenta.transcriptome.genes.shape)} human ortholog
  genes.}
