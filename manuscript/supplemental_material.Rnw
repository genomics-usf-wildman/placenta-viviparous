...
opts_chunk$set(dev="cairo_pdf",out.width="\\textwidth",out.height="0.6\\textheight",out.extra="keepaspectratio,center")
...
\subsection{Orthology types in each Species}

\begin{table}[h]
  \footnotesize
<<orthologs_in_each_species,echo=FALSE,results="asis">>=
load("../data/orthologs_by_species_ranking")
orthologs.by.species[,percentage:=ratio*100]
orthologs.by.species[,orthology_type:=
                          factor(orthology_type,
                                 levels=c("one2one",
                                          "one2many",
                                          "many2many",
                                          "novel/unknown"))]
orthology.wide.table <-
    data.table(dcast(orthologs.by.species,
                     type+orthology_type~species,
                     value.var="percentage"))
setnames(orthology.wide.table,
         "type","Cutoff")
orthology.wide.table[,Cutoff:=xtable.sanitize.text(Cutoff)]
orthology.wide.table[,orthology_type:=xtable.sanitize.text(orthology_type)]
setnames(orthology.wide.table,
         "orthology_type","Orthology Type")
setnames(orthology.wide.table,
         colnames(orthology.wide.table)[-(1:2)],
         paste0("\\textit{",
                gsub("^(.)","\\U\\1",
                     colnames(orthology.wide.table)[-(1:2)],
                     perl=TRUE),
                "}"))
print(xtable(orthology.wide.table,
             digits=0
             ),
...
@ 
\caption{Percentage of orthology in the top 100 and top 1\% of
  placenta transcripts as identified by Ensembl. If a gene has the
  same orthology in comparison to all sequenced species, it is
  reported as that orthology; if it had an orthology of many2many in
  any species, it is reported as many2many; if it had an orthology of
  one2many in any species, but no many2many, it is reported as
  one2many. Novel/unknown genes had no reported orthologies to any
  sequenced species.}
\label{tab:orthologs_in_each_species}
\end{table}
...
placenta.lineage.specific.table <- function(analysis,output=TRUE,
                                            lineage.table=placenta.lineage.specific.analysis,
                                            lit.mining=NULL,
                                            lit.mining.terms=NULL,
                                            members=40,
                                            orderbyfc=FALSE,
                                            fdr.cutoff=1,caption=NULL,
                                            label=NULL) {
    temp.table <- 
    ...
    if (!is.null(lit.mining)) {
        temp.table <-
            lit.mining[,list(human_name,`Adverse Pregnancy Outcomes`)
                       ][temp.table]
        temp.table[,
                   `Adv. Preg. Pub.`:="0"
                   ]
        ...
        temp.table[,`Adverse Pregnancy Outcomes`:=NULL]
    }
    keep.cols <-
        c("Approved Symbol","Approved Name","p.value","fdr","fc")
    if (!is.null(lit.mining)) {
        keep.cols <- c(keep.cols,"Adv. Preg. Pub.")
    }
    temp.table <- 
        hugo.nomenclature[temp.table][,keep.cols,with=FALSE]
    setnames(temp.table,
             c("Approved Symbol","Approved Name","p.value","fdr","fc"),
    ...
    if (output) {
        ...
        col.disp <-
            c("s","s","e","g","f")
        col.align <-
            c("c",">{\\centering\\arraybackslash}p{1in}","r","r","r")
        if (!is.null(lit.mining)) {
            col.disp <- c(col.disp,"s")
            col.align <- c(col.align,">{\\centering\\arraybackslash}p{0.75in}")
        }
        if (orderbyfc) {
...
\subsection{\human specific expression}

<<human.lineage.specific,echo=FALSE,results="asis",cache.extra=file.info("../literature_mining/human_lineage_vs_terms.txt")[,"mtime"]>>=
literature.mining <-
...
hum.all.lin.spec <-
    placenta.lineage.specific.table("hum.all",
                                    ...
                                    lit.mining=literature.mining,
                                    lit.mining.terms=literature.mining.terms,
                                    members=NA,
                                    fdr.cutoff=0.05,
                                    orderbyfc=TRUE,
...
\subsection{\mouse specific expression}

<<mouse.lineage.specific,echo=FALSE,results="asis",cache.extra=file.info("../literature_mining/mouse_lineage_vs_terms.txt")[,"mtime"]>>=
mouse.literature.mining <-
    data.table(read.table("../literature_mining/mouse_lineage_vs_terms.txt",
                          sep="\t",skip=0,stringsAsFactors=FALSE,
                          check.names=FALSE,header=TRUE),
               keep.rownames=TRUE)
setnames(mouse.literature.mining,"rn","human_name")
setkey(mouse.literature.mining,"human_name")
mouse.literature.mining[,`Adverse Pregnancy Outcomes`:=`preeclampsia`+
                             IUGR+`placenta previa`+`gestational diabetes`+
                             `premature birth`,
                        by=human_name]
mou.all.lin.spec <-
    placenta.lineage.specific.table("mou.all",
                                    lineage.table=human.mouse.lineage.specific.analysis,
                                    lit.mining=mouse.literature.mining,
                                    lit.mining.terms=literature.mining.terms,
                                    fdr.cutoff=0.05,
                                    members=NA,
                                    orderbyfc=TRUE,
                                    caption=c("\\lineagetableexplanationtypeiii{\\mouse}{all other species}","\\mouse lineage specific genes"),
                                    label="tab:mouse_lineage_specific_all"
                                    )
...
\subsection{Rodentia, Laurasiatheria, and Euarchontoglires Comparisons}

\begin{table}[hb]
  \footnotesize
<<rodentia.lineage.specific,echo=FALSE,results="asis">>=
rod.all.lin.spec <- placenta.lineage.specific.table("rod.all",fdr.cutoff=0.5)
...
lau.all.lin.spec <- placenta.lineage.specific.table("lau.all",fdr.cutoff=1)
...
eua.lau.lin.spec <- placenta.lineage.specific.table("eua.lau",fdr.cutoff=1)
...
cptlh <- cptlh[,median_FPKM:=median(FPKM),by=human_name]
cptlh <- cptlh[,facet:="high"]
cptlh <- cptlh[median_FPKM < median(median_FPKM),facet:="low"]
...
print(xtable(core.placenta.genes.tsi[order(-`Median FPKM`),
             ...
             label="tab:tissue_specificity_index_of_placenta_core_genes"
             ),
      include.rownames=FALSE,
      booktabs=TRUE,
      math.style.exponents=TRUE,
...
print(ggplot(cpt.h.distribution.shape[`Expression (FPKM)` > 0.1,],
      ...
      + geom_segment(mapping=aes(y=V1-ifelse(V1<170,40,0),
                         yend=V1,
                         xend=10,
                         x=sapply(V1,
                                  function(x){
...
\caption[Core Placenta Pathway Enrichment]{Pathways with significant and suggestive enrichment
  ...
  biocarta, netpath, and kegg pathway databases}
...
\subsection{Tree-based Core Placenta Transcriptome}
...
\begin{figure}[tbh]
<<core_placenta_tree_transcriptome,fig.width=12,fig.height=8,out.width="\\textwidth",out.height="0.8\\textheight",echo=FALSE>>=
cpttl <-
    core.placenta.tree.transcriptome.long[!grepl("(min|median|max|all)_expression|all_but_",
                                                 Species) & housekeeping==FALSE,]
cpttl <- cpttl[!is.na(tree_genes),Species:=factor(gsub("^([^ ])[^ ]* +([^ ]*)","\\1. \\2",capfirst(Species)),
                                levels=gsub("^([^ ])[^ ]* +([^ ]*)","\\1. \\2",
                                            capfirst(names(species.names))))]
cpttl <- cpttl[,median_FPKM:=median(FPKM,na.rm=TRUE),by=tree_genes]
cpttl <- cpttl[,facet:="high"]
cpttl <- cpttl[median_FPKM < median(median_FPKM),facet:="low"]
print(ggplot(cpttl[FPKM > 0.1,],aes(x=reorder(tree_genes,median_FPKM),y=FPKM))+
      geom_boxplot(outlier.shape=NA)+
      scale_y_log10(breaks=c(10^1,10^2,10^3,10^4),
                    labels=trans_format("log10", math_format(10^.x)))+
      geom_point(aes(color=Species,shape=Species))+
...
@ 
\caption[Tree-based Core Placenta Transcriptome]{
  Tree-based Core Placenta Transcriptome.
  \Sexpr{cpttl[,length(unique(tree_genes))]}
  gene trees with mean expression ($\mathrm{FPKM} \ge 10$) in the
  placenta of all \Sexpr{star.logs[,length(unique(species))]} species
  studied which are not housekeeping~%
  \cite{Eisenberg.ea2013:Humanhousekeepinggenesrevisited}
  ordered (from top to bottom) by median expression.}
\label{fig:core_placenta_tree_transcriptome}
\end{figure}

\begin{figure}
<<core_placenta_tree_transcriptome_housekeeping,fig.width=12,fig.height=12,out.height="0.8\\textheight",echo=FALSE>>=
cpttlh <-
    core.placenta.tree.transcriptome.long[!grepl("(min|median|max|all)_expression|all_but_",
                                                 Species) & housekeeping==TRUE,]
cpttlh <- cpttlh[,Species:=factor(gsub("^([^ ])[^ ]* +([^ ]*)","\\1. \\2",capfirst(Species)),
                                levels=gsub("^([^ ])[^ ]* +([^ ]*)","\\1. \\2",
                                            capfirst(names(species.names))))]

cpttlh <- cpttlh[,median_FPKM:=median(FPKM,na.rm=TRUE),by=tree_genes]
cpttlh <- cpttlh[,facet:="high"]
cpttlh <- cpttlh[median_FPKM < median(median_FPKM),facet:="low"]
print(ggplot(cpttlh[FPKM > 0.1,],aes(x=reorder(tree_genes,median_FPKM),y=FPKM))+
      geom_boxplot(outlier.shape=NA)+
      scale_y_log10(breaks=c(10^1,10^2,10^3,10^4),
                    labels=trans_format("log10", math_format(10^.x)))+
      geom_point(aes(color=Species,shape=Species))+
...
@ 
\caption[Tree-based Core Placenta Transcriptome: Housekeeping Genes]{
  Tree-based Core Placenta Transcriptome: Housekeeping Genes.
  \Sexpr{cpttlh[,length(unique(tree_genes))]}
  gene trees with mean expression ($\mathrm{FPKM} \ge 10$) in the
  placenta of all \Sexpr{star.logs[,length(unique(species))]} species
  studied which are housekeeping~%
  \cite{Eisenberg.ea2013:Humanhousekeepinggenesrevisited}
  ordered (from top to bottom) by median expression.}
\label{fig:core_placenta_tree_transcriptome_housekeeping}
\end{figure}
...
\begin{figure}
<<actin_pathway,echo=FALSE,results="hide",message=FALSE,warning=FALSE>>=
plot_pathway("hsa04810",pathcolor=quote(log10(median_expression+1)),cutpoint=0.1)
#library("pathview")
...
%  \includegraphics{hsa04810.pathview.png}
  \caption{KEGG Actin Cytoskeleton Pathway. Expression in $\log_{10}$ FPKM.}
