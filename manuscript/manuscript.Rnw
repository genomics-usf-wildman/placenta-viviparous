...
pathway. Two other pathways, the mPR/Progesterone pathway and Rho cell
motility pathways, feed into the Actin Y pathway as well. Supplemental
\cref{fig:egf_kegg_pathway} shows the EGF pathway in humans with the
median expression in FPKM of genes in the pathway shown. Notably, many
of the genes on the GRB2→MYC pathway are strongly expressed; their
activation leads to an increase in migration and invasion in response
to EGF binding to EGFR1. The Actin cytoskeleton pathway is depicted in
Supplemental \cref{fig:kegg_actin}, which shows that \textit{FN1} and
\textit{ITG} are highly expressed, which activate Rac, and potentially
lead to the formation of focal adhesions and lamellipodia formation
through their inhibition of GSN and HSPC300.
...
\textit{WNT5A}, which inhibits the invasion of the trophoblast cell
line HTR8/SVneo \textit{in vitro} \DLA{Cite this paper after reading
  the comments on it (PMID: 26865089)}.
...
\subsubsection{Immune Regulation}
...
promoting the fusion of cells~\DLA{Cite more papers here which define
  this.} in the presence of Ca$^{2+}$. While there are likely other
genes which are responsible for the positioning of cytotrophoblast
cells in close proximity to start the fusion process, \textit{ANXA5}
is likely to be essential for the completion of fusion.
