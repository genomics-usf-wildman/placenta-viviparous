...
\newcommand{\horse}[0]{\textit{E. galili}\xspace}
...
placenta.lineage.specific.table <- function(analysis,output=TRUE) {
    ...
    setkey(temp.table,"Approved Symbol")
    setnames(temp.table,
              c("Human Symbol","Human Name","p (t-test)","FDR","Fold Change")
              )
    if (output) {
        print(xtable(temp.table[order(p.value),],
    ...
    return(invisible(temp.table[order(p.value),]))
}
@ 
